    ...
    */
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
            ...
            for (int i_pi0 = 0; i_pi0 < N_pi0_syst; i_pi0++) Correction_pi0 = Correction_pi0 * std::pow(pi0_correction[i_pi0], temp_N_bin_pi0[i_pi0]);
            for (int i_fake = 0; i_fake < N_fakeE_syst; i_fake++) {
                Correction_fake = Correction_fake * std::pow(PID_fakeE_correction[0][i_fake] * PID_fakeE_correction_fluctuated[0][i_fake], temp_N_bin_fakeE[0][i_fake]); // K- from e
                Correction_fake = Correction_fake * std::pow(PID_fakeE_correction[1][i_fake] * PID_fakeE_correction_fluctuated[1][i_fake], temp_N_bin_fakeE[1][i_fake]); // K+ from e
                Correction_fake = Correction_fake * std::pow(PID_fakeE_correction[2][i_fake] * PID_fakeE_correction_fluctuated[2][i_fake], temp_N_bin_fakeE[2][i_fake]); // pi- from e
                Correction_fake = Correction_fake * std::pow(PID_fakeE_correction[3][i_fake] * PID_fakeE_correction_fluctuated[3][i_fake], temp_N_bin_fakeE[3][i_fake]); // pi+ from e
            }
            for (int i_fake = 0; i_fake < N_fakeMU_syst; i_fake++) {
                Correction_fake = Correction_fake * std::pow(PID_fakeMU_correction[0][i_fake] * PID_fakeMU_correction_fluctuated[0][i_fake], temp_N_bin_fakeMU[0][i_fake]); // K- from mu
                Correction_fake = Correction_fake * std::pow(PID_fakeMU_correction[1][i_fake] * PID_fakeMU_correction_fluctuated[1][i_fake], temp_N_bin_fakeMU[1][i_fake]); // K+ from mu
                Correction_fake = Correction_fake * std::pow(PID_fakeMU_correction[2][i_fake] * PID_fakeMU_correction_fluctuated[2][i_fake], temp_N_bin_fakeMU[2][i_fake]); // pi- from mu
                Correction_fake = Correction_fake * std::pow(PID_fakeMU_correction[3][i_fake] * PID_fakeMU_correction_fluctuated[3][i_fake], temp_N_bin_fakeMU[3][i_fake]); // pi+ from mu
            }
...
void FluctuatePIDCorrection(bool IsItKID) {
    ...
    for (int i_fake = 0; i_fake < N_fakeE_syst; i_fake++) {
        ...
        std::lognormal_distribution<double> Kp_fakeE_distribution(0.0, PID_fakeE_uncer[1][i_fake] / PID_fakeE_correction[1][i_fake]);
        std::lognormal_distribution<double> Kn_fakeE_distribution(0.0, PID_fakeE_uncer[0][i_fake] / PID_fakeE_correction[0][i_fake]);
        std::lognormal_distribution<double> pip_fakeE_distribution(0.0, PID_fakeE_uncer[3][i_fake] / PID_fakeE_correction[3][i_fake]);
        std::lognormal_distribution<double> pin_fakeE_distribution(0.0, PID_fakeE_uncer[2][i_fake] / PID_fakeE_correction[2][i_fake]);

        if (IsItKID) {
            if (std::abs(PID_fakeE_uncer[1][i_fake] / PID_fakeE_correction[1][i_fake]) < MyEPSILON) PID_fakeE_correction_fluctuated[1][i_fake] = 1.0;
            else PID_fakeE_correction_fluctuated[1][i_fake] = Kp_fakeE_distribution(generator);
            if (std::abs(PID_fakeE_uncer[0][i_fake] / PID_fakeE_correction[0][i_fake]) < MyEPSILON) PID_fakeE_correction_fluctuated[0][i_fake] = 1.0;
            ...
            if (std::abs(PID_fakeE_uncer[3][i_fake] / PID_fakeE_correction[3][i_fake]) < MyEPSILON) PID_fakeE_correction_fluctuated[3][i_fake] = 1.0;
            else PID_fakeE_correction_fluctuated[3][i_fake] = pip_fakeE_distribution(generator);
            if (std::abs(PID_fakeE_uncer[2][i_fake] / PID_fakeE_correction[2][i_fake]) < MyEPSILON) PID_fakeE_correction_fluctuated[2][i_fake] = 1.0;
    ...
    for (int i_fake = 0; i_fake < N_fakeMU_syst; i_fake++) {
        ...
        std::lognormal_distribution<double> Kp_fakeMU_distribution(0.0, PID_fakeMU_uncer[1][i_fake] / PID_fakeMU_correction[1][i_fake]);
        std::lognormal_distribution<double> Kn_fakeMU_distribution(0.0, PID_fakeMU_uncer[0][i_fake] / PID_fakeMU_correction[0][i_fake]);
        std::lognormal_distribution<double> pip_fakeMU_distribution(0.0, PID_fakeMU_uncer[3][i_fake] / PID_fakeMU_correction[3][i_fake]);
        std::lognormal_distribution<double> pin_fakeMU_distribution(0.0, PID_fakeMU_uncer[2][i_fake] / PID_fakeMU_correction[2][i_fake]);

        if (IsItKID) {
            if (std::abs(PID_fakeMU_uncer[1][i_fake] / PID_fakeMU_correction[1][i_fake]) < MyEPSILON) PID_fakeMU_correction_fluctuated[1][i_fake] = 1.0;
            else PID_fakeMU_correction_fluctuated[1][i_fake] = Kp_fakeMU_distribution(generator);
            if (std::abs(PID_fakeMU_uncer[0][i_fake] / PID_fakeMU_correction[0][i_fake]) < MyEPSILON) PID_fakeMU_correction_fluctuated[0][i_fake] = 1.0;
            ...
            if (std::abs(PID_fakeMU_uncer[3][i_fake] / PID_fakeMU_correction[3][i_fake]) < MyEPSILON) PID_fakeMU_correction_fluctuated[3][i_fake] = 1.0;
            else PID_fakeMU_correction_fluctuated[3][i_fake] = pip_fakeMU_distribution(generator);
            if (std::abs(PID_fakeMU_uncer[2][i_fake] / PID_fakeMU_correction[2][i_fake]) < MyEPSILON) PID_fakeMU_correction_fluctuated[2][i_fake] = 1.0;
...
void PID_calculator()
{
    ...
    for (int i = 0; i < NToys; i++) {
        ...
        GetFlucNevt(MC_dirname_Knn, temp_hist, "Bplus", "Knn", Nevt_fluc_KID, i, true, 1.0, "B2Knn");
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_Kstarnn, temp_hist, "Bplus", "Kstarnn", Nevt_fluc_KID, i, true, 1.0, "B2Kstarnn");
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_K0nn, temp_hist, "Bzero", "K0nn", Nevt_fluc_KID, i, true, 1.0, "B02K0nn");
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_K0starnn, temp_hist, "Bzero", "K0starnn", Nevt_fluc_KID, i, true, 1.0, "B02K0starnn");
        temp_hist->Reset();
    }
    /* ====================================== */
    ...
    for (int i = 0; i < NToys; i++) {
        ...
        GetFlucNevt(MC_dirname_Knn, temp_hist, "Bplus", "Knn", Nevt_fluc_PID, i, false, 1.0, "B2Knn");
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_Kstarnn, temp_hist, "Bplus", "Kstarnn", Nevt_fluc_PID, i, false, 1.0, "B2Kstarnn");
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_K0nn, temp_hist, "Bzero", "K0nn", Nevt_fluc_PID, i, false, 1.0, "B02K0nn");
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_K0starnn, temp_hist, "Bzero", "K0starnn", Nevt_fluc_PID, i, false, 1.0, "B02K0starnn");
        temp_hist->Reset();
    }
    /* ====================================== */

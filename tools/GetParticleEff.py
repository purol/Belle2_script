...
ma.variablesToNtuple(decayString="K+:Kaon_true", variables = ["p", "cosTheta", "kaonID", "pionID"] + ["averageValueInList(Upsilon(4S):PrimaryMC, qsquared)", "averageValueInList(Upsilon(4S):PrimaryMC, mcDaughter(0, mcDaughter(0, M)))"], filename=output_file, treename="Kaon", path=my_path)
ma.variablesToNtuple(decayString="pi+:Pion_true", variables = ["p", "cosTheta", "kaonID", "pionID"] + ["averageValueInList(Upsilon(4S):PrimaryMC, qsquared)", "averageValueInList(Upsilon(4S):PrimaryMC, mcDaughter(0, mcDaughter(0, M)))"], filename=output_file, treename="Pion", path=my_path)

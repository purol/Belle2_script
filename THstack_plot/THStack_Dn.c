...
THStack* Stack = new THStack("thstack", ";D^{0} candidate mass [GeV];number of candidates");
TH1D* SIGNAL_hist = new TH1D("signal #times 3000", ";D^{0} candidate mass [GeV];number of candidates", 70, 1.5, 2.0);
TH1D* CHG_hist = new TH1D("charged", ";D^{0} candidate mass [GeV];number of candidates", 70, 1.5, 2.0);
TH1D* MIX_hist = new TH1D("mixed", ";D^{0} candidate mass [GeV];number of candidates", 70, 1.5, 2.0);
TH1D* UUBAR_hist = new TH1D("u#bar{u}", ";D^{0} candidate mass [GeV];number of candidates", 70, 1.5, 2.0);
TH1D* DDBAR_hist = new TH1D("d#bar{d}", ";D^{0} candidate mass [GeV];number of candidates", 70, 1.5, 2.0);
TH1D* SSBAR_hist = new TH1D("s#bar{s}", ";D^{0} candidate mass [GeV];number of candidates", 70, 1.5, 2.0);
TH1D* CHARM_hist = new TH1D("c#bar{c}", ";D^{0} candidate mass [GeV];number of candidates", 70, 1.5, 2.0);
...
void LetsFill(const char* dirname, TH1D* hist, double weight = 1) {
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        tree_Bsig->SetBranchAddress("Bsig_daughter_0_extraInfo_Dnvetomass", &var);

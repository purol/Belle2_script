...
plot_title = '$\mu^{-}$ ratio table for cut "pionID>0.6"'
fig, ax = plt.subplots(1, 1, figsize=(7, 5), dpi=120)

df = pd.read_csv("./pionID_efficiency_muon.csv")

x_label_name = "theta"
y_label_name = "p"
x_label = [0.4, 0.64, 0.82, 1.16, 1.46, 1.78, 2.13, 2.22, 2.6] # theta
y_label = [0.4, 0.5, 1.0, 1.5, 2, 2.5] # p
...
arr = np.empty((len(y_label) - 1,len(x_label) - 1,))
arr[:] = np.nan
...
plt.savefig('MU_minus_pionID.png')

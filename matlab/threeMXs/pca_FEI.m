...
FEIfileID_w = fopen('FEI_MX1_selected.txt','w');
for j = 1:N_selected_vectors
    fprintf(FEIfileID_w,"%1.15f\n", sqrt(FEIlatent(j)));
    for i = 1:6
        temp_eigen_vector = FEIcoeff(:,j);
...
FEIfileID_w = fopen('FEI_MX2_selected.txt','w');
for j = 1:N_selected_vectors
    fprintf(FEIfileID_w,"%1.15f\n", sqrt(FEIlatent(j)));
    for i = 7:12
        temp_eigen_vector = FEIcoeff(:,j);
...
FEIfileID_w = fopen('FEI_MX3_selected.txt','w');
for j = 1:N_selected_vectors
    fprintf(FEIfileID_w,"%1.15f\n", sqrt(FEIlatent(j)));
    for i = 13:18
        temp_eigen_vector = FEIcoeff(:,j);
...
FEIfileID_w = fopen('FEI_MX1_cov_remain_truncated.txt','w');
for i = 1:6
    fprintf(FEIfileID_w,"%1.15f\n",FEIremaincov(i,i));
end
for i = 19:24
...
FEIfileID_w = fopen('FEI_MX2_cov_remain_truncated.txt','w');
for i = 6:12
    fprintf(FEIfileID_w,"%1.15f\n",FEIremaincov(i,i));
end
for i = 25:30
...
FEIfileID_w = fopen('FEI_MX3_cov_remain_truncated.txt','w');
for i = 13:18
    fprintf(FEIfileID_w,"%1.15f\n",FEIremaincov(i,i));
end
for i = 31:36

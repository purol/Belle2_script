...
void THStack_plot_nominal_FBDT() {
    ...
    */
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // fill
        ...
        for (int i = 0; i < (int)Nominal_MC_values[k].size(); i++) MC_one_bin[k]->Fill(Nominal_MC_values[k].at(i), weights.at(i));
        for (int i = 0; i < (int)Nominal_data_values[k].size(); i++) data_one_bin[k]->Fill(Nominal_data_values[k].at(i));
    }

...
#include "generators/evtgen/models/EvtBtoXsnunu.h"
...
namespace Belle2 {
  ...
  B2_EVTGEN_REGISTER_MODEL(EvtBtoXsnunu);

  EvtBtoXsnunu::~EvtBtoXsnunu()
  {
    delete _calcprob;
  }

  std::string EvtBtoXsnunu::getName()
  {
    return "BTOXSNUNU";
  }

  EvtDecayBase* EvtBtoXsnunu::clone()
  {
    return new EvtBtoXsnunu;
  }

  void EvtBtoXsnunu::decay(EvtParticle* p)
  {
  ...
  while (xhadronMass < _mxmin)
  {
    ...
    //    dout(Debugout::INFO,"EvtBtoXsll") << "b-quark momentum = " << pb << " mass = " <<  mb << std::endl;
    ...
    //    dout(Debugout::INFO,"EvtBtoXsll") << "dGdsProb(s) = " << _calcprob->dGdsProb(mb, _ms, ml, s)
    //         << " for s = " << s << std::endl;
    ...
    while (tmp == 0.0)
    {
      ...
      {
        std::cout << "d2gdsdup GT d2gdsdup_max:" << prob
             << " " << _dGdsdupProbMax
      ...
      {
        tmp = 1.0;
	//        dout(Debugout::INFO,"EvtBtoXsll") << "dGdsdupProb(s) = " << prob
	//             << " for u = " << u << std::endl;
      }
    }
    ...
    //    p4B = boostTo(p4B, p4b);
    //
    //    dout(Debugout::INFO,"EvtBtoXsll") << " B meson mass in b-quark rest frame = " << p4B.mass() << std::endl;
    ...
    //    dout(Debugout::INFO,"EvtBtoXsll") << "Xs mass = " << xhadronMass << " trial " << im << std::endl;
  }
  ...
  void EvtBtoXsnunu::initProbMax()
  {
    noProbMax();
  }


  void EvtBtoXsnunu::init()
  {
  ...
  if ( lepton1type == EvtPDL::getId("anti-nu_e") ||
       lepton1type == EvtPDL::getId("nu_e") ) { etyp++;}
  if ( lepton2type == EvtPDL::getId("anti-nu_e") ||
       lepton2type == EvtPDL::getId("nu_e") ) { etyp++;}
  if ( lepton1type == EvtPDL::getId("anti-nu_mu") ||
       lepton1type == EvtPDL::getId("nu_mu") ) { mutyp++;}
  if ( lepton2type == EvtPDL::getId("anti-nu_mu") ||
       lepton2type == EvtPDL::getId("nu_mu") ) { mutyp++;}
  if ( lepton1type == EvtPDL::getId("anti-nu_tau") ||
       lepton1type == EvtPDL::getId("nu_tau") ) { tautyp++;}
  if ( lepton2type == EvtPDL::getId("anti-nu_tau") ||
       lepton2type == EvtPDL::getId("nu_tau") ) { tautyp++;}
  ...
  if ( lepton1type == EvtPDL::getId("anti-nu_e")  ||
       ...
       lepton1type == EvtPDL::getId("anti-nu_tau") ) { lpos++;}
  if ( lepton2type == EvtPDL::getId("nu_e")  ||
       lepton2type == EvtPDL::getId("nu_mu") ||
       lepton2type == EvtPDL::getId("nu_tau") ) { lneg++;}
  ...
  if ( verbose() ) {
    std::cout << "dGdsProbMax = " << probMax << " for s = "  << sProbMax << std::endl;
  }
  ...
  if ( verbose() ) {
   std::cout << "dGdsdupProbMax = " << probMax << " for s = "  << sProbMax

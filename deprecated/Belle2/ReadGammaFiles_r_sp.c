...
typedef struct data{
    ...
    double variables[3];
    // 0: inCDCAcceptance, 1: clusterNHits, 2: E
...
void Loader::GetData(TFile* input_file) {
    TTree* tree_Gamma = (TTree*)input_file->Get("Gamma");

    Data temp = { 0 };

    // get event_info
    tree_Gamma->SetBranchAddress("__experiment__", &temp.__experiment__);
    tree_Gamma->SetBranchAddress("__run__", &temp.__run__);
    tree_Gamma->SetBranchAddress("__event__", &temp.__event__);
    tree_Gamma->SetBranchAddress("__candidate__", &temp.__candidate__);
    tree_Gamma->SetBranchAddress("__ncandidates__", &temp.__ncandidates__);

    // get Upsilon_info
    tree_Gamma->SetBranchAddress("inCDCAcceptance", &temp.variables[0]);
    tree_Gamma->SetBranchAddress("clusterNHits", &temp.variables[1]);
    tree_Gamma->SetBranchAddress("E", &temp.variables[2]);
    // 0: inCDCAcceptance, 1: clusterNHits, 2: E

    printf("%lld entries...\n", tree_Gamma->GetEntries());
    for (unsigned int j = 0; j < tree_Gamma->GetEntries(); j++) { // Fill
...
void ReadGammaFiles_r_sp(){
    ...
    for(unsigned int i = 0; i<names.size(); i++){
        ...
        loader.DrawTH1F("inCDCAcceptance", "inCDCAcceptance of gamma candidates;inCDCAcceptance;evt", 100, 0, 1, Loader::variables, 0);
        loader.DrawTH1F("clusterNHits", "clusterNHits of gamma candidates;clusterNHits;evt", 100, 0, 5, Loader::variables, 1);
        loader.DrawTH1F("E", "E of gamma candidates;E [GeV];evt", 100, 0, 1.0, Loader::variables, 2);

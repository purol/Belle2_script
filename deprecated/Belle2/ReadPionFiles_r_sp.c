...
void Loader::GetData(TFile* input_file) {
    TTree* tree_Pion = (TTree*)input_file->Get("Pion");

    Data temp = { 0 };

    // get event_info
    tree_Pion->SetBranchAddress("__experiment__", &temp.__experiment__);
    tree_Pion->SetBranchAddress("__run__", &temp.__run__);
    tree_Pion->SetBranchAddress("__event__", &temp.__event__);
    tree_Pion->SetBranchAddress("__candidate__", &temp.__candidate__);
    tree_Pion->SetBranchAddress("__ncandidates__", &temp.__ncandidates__);

    // get Upsilon_info
    tree_Pion->SetBranchAddress("pionID", &temp.variables[0]);
    tree_Pion->SetBranchAddress("nCDCHits", &temp.variables[1]);
    tree_Pion->SetBranchAddress("dr", &temp.variables[2]);
    tree_Pion->SetBranchAddress("dz", &temp.variables[3]);
    tree_Pion->SetBranchAddress("M", &temp.variables[4]);
    tree_Pion->SetBranchAddress("dM", &temp.variables[5]);

    printf("%lld entries...\n", tree_Pion->GetEntries());
    for (unsigned int j = 0; j < tree_Pion->GetEntries(); j++) { // Fill
...
void ReadPionFiles_r_sp(){
    ...
    for(unsigned int i = 0; i<names.size(); i++){
        ...
        loader.DrawTH1F("pionID", "pionID of pion candidates;pionID;evt", 100, 0, 1, Loader::variables, 0);
        loader.DrawTH1F("nCDCHits", "nCDCHits of pion candidates;nCDCHits;evt", 100, 0, 60, Loader::variables, 1);
        loader.DrawTH1F("dr", "dr of pion candidates;dr [cm];evt", 100, 0, 1.5, Loader::variables, 2);
        loader.DrawTH1F("dz", "dz of pion candidates;dz [cm];evt", 100, -4, 4, Loader::variables, 3);
        loader.Cut(Loader::variables, 0, Loader::larger_than, 0.4);
        loader.Cut(Loader::variables, 1, Loader::larger_than, 20);
        loader.Cut(Loader::variables, 2, Loader::smaller_than, 0.5);
        loader.Cut(Loader::variables, 3, Loader::smaller_than, 2.0);
        loader.Cut(Loader::variables, 3, Loader::larger_than, -2.0);

        loader.PrintInformation(std::string("========== pionID > 0.4 & nCDCHits > 20 & dr < 0.5 & abs(dz) < 2.0 =========="));
        loader.DrawTH1F("M_after_cuts", "M of pion candidates;M [GeV];evt", 100, 0.1385, 0.1405, Loader::variables, 4);
    }
    loader.End();
}

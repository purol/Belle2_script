...
void LetsFillEecl(const char* dirname, TH1D* hist_eecl[NTest], TH1D* hist_ngamma[NTest], std::string SampleName) {
    /*
    0: charged
    1: mixed
    2: uubar
...
void LetsFillEecl(const char* dirname, TH1D* hist_eecl, TH1D* hist_ngamma) {
    ...
    */
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        tree_upsilon->SetBranchAddress("extraInfo__boEeclv200__bc", &Eecl_var);
        tree_upsilon->SetBranchAddress("extraInfo__boNgammav200__bc", &Ngamma_var);
        ...
        for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
            ...
            hist_eecl->Fill(Eecl_var);
            hist_ngamma->Fill(Ngamma_var);
...
void CalculateChiSquared(TH1D* hist_eecl[NTest], TH1D* hist_ngamma[NTest], TH1D* hist_eecl_data, TH1D* hist_ngamma_data) {
    ...
    for (int k = 0; k < NTest; k++) {
        ...
        ChiSquareds[k] = ChiSquared;
    }

    TGraph* gr = new TGraph(NTest, Scales, ChiSquareds);
    gr->SetTitle(";f_{h};#chi^{2}/dof");
...
void CalculateKSProb(TH1D* hist_eecl[NTest], TH1D* hist_ngamma[NTest], TH1D* hist_eecl_data, TH1D* hist_ngamma_data) {
    ...
    TCanvas* c_temp = new TCanvas("c", "", 800, 1200); c_temp->cd();
    gr->Draw("AL");
...
void FakePhotonCalculator(){
    ...
    for (int i = 0; i < NTest; i++) {
        Eecl_v200[i] = new TH1D(("Eecl_v200_" + to_string(Scales[i])).c_str(), ";E_{ecl} [GeV];number of candidates", NBin, 0, 6.0);
        Ngamma_v200[i] = new TH1D(("Ngamma_v200_" + to_string(Scales[i])).c_str(), ";Number of gamma candidates;number of candidates", NBin, 0, 25.0);
    }
    ...
    LetsFillEecl(Sideband_MC_CHG_train_dirname, Eecl_v200, Ngamma_v200, "CHG");
    LetsFillEecl(Sideband_MC_MIX_train_dirname, Eecl_v200, Ngamma_v200, "MIX");
    LetsFillEecl(Sideband_MC_UUBAR_train_dirname, Eecl_v200, Ngamma_v200, "UUBAR");
    LetsFillEecl(Sideband_MC_DDBAR_train_dirname, Eecl_v200, Ngamma_v200, "DDBAR");
    LetsFillEecl(Sideband_MC_SSBAR_train_dirname, Eecl_v200, Ngamma_v200, "SSBAR");
    LetsFillEecl(Sideband_MC_CHARM_train_dirname, Eecl_v200, Ngamma_v200, "CHARM");
    LetsFillEecl(Sideband_MC_CHG_test_dirname, Eecl_v200, Ngamma_v200, "CHG");
    LetsFillEecl(Sideband_MC_MIX_test_dirname, Eecl_v200, Ngamma_v200, "MIX");
    LetsFillEecl(Sideband_MC_UUBAR_test_dirname, Eecl_v200, Ngamma_v200, "UUBAR");
    LetsFillEecl(Sideband_MC_DDBAR_test_dirname, Eecl_v200, Ngamma_v200, "DDBAR");
    LetsFillEecl(Sideband_MC_SSBAR_test_dirname, Eecl_v200, Ngamma_v200, "SSBAR");
    LetsFillEecl(Sideband_MC_CHARM_test_dirname, Eecl_v200, Ngamma_v200, "CHARM");

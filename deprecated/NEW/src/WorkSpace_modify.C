...
int WorkSpace_modify() {
	...
	// get BR for each MXs^true region
	...
	// set mu_MXs1 to be constant
	mu_MXs1->setConstant(true);

	// define \mu and define another mu_MXs1
	RooRealVar* mu = new RooRealVar("mu", "mu", 1.0, -100.0, 100.0);
	RooFormulaVar* mu1 = new RooFormulaVar("mu_MXs1_alt", "@2*5.98159294224-@0*1.75256626953-@1*3.22902667271", RooArgList(*mu_MXs2, *mu_MXs3, *mu));

	// use another mu_MXs1 as the scale factor
	...
	// set POI and parameter to be \mu, instead of mu_MXs1
	w->import(*mu);
	w->import(*mu1);
	sbModel->SetParametersOfInterest(RooArgSet(*mu));
	sbModel->SetParameters(RooArgSet(*mu));
	//sbModel->GetPdf()->getVariables()->add(RooArgSet(*mu));
	//const RooArgSet* POI = sbModel->GetParametersOfInterest();
	//POI->Print();

	//RooDataSet* data = (RooDataSet*)w->data("asimovData");
	//sbModel->GetPdf()->getParameters(*data)->add(RooArgSet(*mu));
	//sbModel->GetPdf()->getComponents();

	// set snapshot
	w->saveSnapshot("NominalParamValues", w->allVars());


	//w->Print();
	//w->writeToFile("PDFandDATA_workspace_mu.root");

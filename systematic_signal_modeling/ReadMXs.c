...
void LetsFillMXs(const char* dirname, TH1D* hist_q2, Corrector_Fragmentation::SystType systtype) {
...
void ReadMXs(){
    ...
    TH1D* Xsu_hist = new TH1D("Xsu_MXs", ";M_{Xs} [GeV/c^{2}]; evt", Nentry, MIN_MXs, MAX_MXs);
    TH1D* Xsd_hist = new TH1D("Xsd_MXs", ";M_{Xs} [GeV/c^{2}]; evt", Nentry, MIN_MXs, MAX_MXs);

    LetsFillMXs(Xsu_dirname, Xsu_hist, Corrector_Fragmentation::SystType::Nominal);
    LetsFillMXs(Xsd_dirname, Xsd_hist, Corrector_Fragmentation::SystType::Nominal);

    Double_t factor = 1.;
    Xsu_hist->Scale(factor / Xsu_hist->Integral(), "width");
    Xsd_hist->Scale(factor / Xsd_hist->Integral(), "width");

    FILE* fp;
    fp = fopen("Xsu_distribution.csv","w");
    fprintf(fp,"MXs_min,MXs_max,value\n");
    for (int j = 0; j < Nentry; j++) {
        double value = Xsu_hist->GetBinContent(j + 1);
        fprintf(fp, "%lf,%lf,%lf\n", MIN_MXs + j * (MAX_MXs - MIN_MXs) / Nentry, MIN_MXs + (j + 1) * (MAX_MXs - MIN_MXs) / Nentry, value);
    }
    fclose(fp);

    fp = fopen("Xsd_distribution.csv", "w");
    fprintf(fp, "MXs_min,MXs_max,value\n");
    for (int j = 0; j < Nentry; j++) {
        double value = Xsd_hist->GetBinContent(j + 1);
        fprintf(fp, "%lf,%lf,%lf\n", MIN_MXs + j * (MAX_MXs - MIN_MXs) / Nentry, MIN_MXs + (j + 1) * (MAX_MXs - MIN_MXs) / Nentry, value);
    }
    fclose(fp);

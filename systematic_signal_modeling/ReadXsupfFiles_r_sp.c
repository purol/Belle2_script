...
void ReadXsupfFiles_r_sp(){
    const char* Xsu_Hpf_dirname = "./Xsu_Hpf";
    const char* Xsu_Lpf_dirname = "./Xsu_Lpf";
    ...
    LetsFillMXs(Xsu_Hpf_dirname, hist_Hpf, Corrector_Fragmentation::SystType::Hpf);
    LetsFillMXs(Xsu_Lpf_dirname, hist_Lpf, Corrector_Fragmentation::SystType::Lpf);
    LetsFillMXs(Xsu_dirname, hist, Corrector_Fragmentation::SystType::Nominal);
    ...
    hist->Draw("SAME Hist");
    legend->Draw("SAME");
    c_temp->SaveAs("Xsu_pf.png");

    FILE* fp;
    fp = fopen("Xsu_Hpf_weight.txt","w");

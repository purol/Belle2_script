...
void LetsFillMXs(const char* dirname, TH1D* hist_q2, Corrector_Fragmentation::SystType systtype) {
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        for (unsigned int j = 0; j < tree_Xs->GetEntries(); j++) { // Fill
            ...
            double Fragmentation_reweight = corrector_Fragmentation.GetCorrectionFactor(Decay, Mxs_B0_MC, systtype, Corrector_Fragmentation::Sample::gamma, MCTYPE);

            hist_q2->Fill(Mxs_B0_MC, Fragmentation_reweight);
...
void ReadXsdpfFiles_r_sp(){
    ...
    LetsFillMXs(Xsd_Hpf_dirname, hist_Hpf, Corrector_Fragmentation::SystType::Hpf);
    LetsFillMXs(Xsd_Lpf_dirname, hist_Lpf, Corrector_Fragmentation::SystType::Lpf);
    LetsFillMXs(Xsd_dirname, hist, Corrector_Fragmentation::SystType::Nominal);

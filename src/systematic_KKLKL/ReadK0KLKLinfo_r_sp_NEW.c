...
void FillHist() {
    ...
    for (size_t i = 0; i < Infos_PHSP.size(); i++) {
        double smax_ = Infos_PHSP.at(i).smax;
        double smin_ = Infos_PHSP.at(i).smin;
        double smed_ = Infos_PHSP.at(i).smed;
    ...
    for (size_t i = 0; i < Infos_phi.size(); i++) {
        double smax_ = Infos_phi.at(i).smax;
        double smin_ = Infos_phi.at(i).smin;
        double smed_ = Infos_phi.at(i).smed;

        int GLobalBin = 0;

        // find Bin index
        GLobalBin = Prob_phi->FindBin(smax_, smin_);
        Prob_phi->SetBinContent(GLobalBin, Prob_phi->GetBinContent(GLobalBin) + 1.0);
...
int main(){
    ...
    This code care about B0 --> phi(-> KS0 KL0) KL0
    */
    
    const char* dirname_PHSP = "/home/belle2/junewoo/storage_ghi/20230924_B02KKLKL_generator_level_info/output/Ntuple";

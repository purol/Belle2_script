...
# define Nbin_MXs_true 3

/* ====================================== */

void GetNominalNevt(const char* dirname, const char* included_string, const char* type, const char* sample, TH2D* mapping, double weight_var = 1.0, std::string CorrectionType = "otherwise") { // get nominal PDF with appropriate correction
    /*
    ...
    */
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
            ...
            // find reco index
            if ((Bsig_M > 0.0) && (Bsig_M < 0.6)) MXs_reco_index = 0;
            else if ((Bsig_M > 0.6) && (Bsig_M < 1.0)) MXs_reco_index = 1;
            else if ((Bsig_M > 1.0) && (Bsig_M < 2.0)) MXs_reco_index = 2;
            else {
                printf("undexpected Bsig_M\n");
                exit(1);
            }
            ...
            if ((MC_MXs > 0.0) && (MC_MXs < 0.6)) MXs_true_index = 0;
            else if ((MC_MXs > 0.6) && (MC_MXs < 1.0)) MXs_true_index = 1;
            else if ((MC_MXs > 1.0) && (MC_MXs < 6.0)) MXs_true_index = 2;

            // apply FBDT correction
            if (MXs_reco_index == 0) total_weight = total_weight * 1.0045;
            else if (MXs_reco_index == 1) total_weight = total_weight * 1.0460;
            else if (MXs_reco_index == 1) total_weight = total_weight * 0.9717;

            mapping->Fill(MXs_true_index + 0.5, ReturnBinIndex(MVA_var, Bsig_M), total_weight);
...
int main(int argc, char* argv[]) {
    // function to calculate cross feed matrix

    TH2D* mapping = new TH2D("mapping", ";;", 3, 0, 3, 15, 0, 15); // <true bin, bin index>

    const char* dirname_SIGNAL = "/home/belle2/junewoo/storage_ghi/Analysis/MizuhashiRD/SIGNAL_analysis/validation_v009/final_output_root_after_MVA_Application_after_cut/Merge";

    GetNominalNevt(dirname_SIGNAL, "B2Knunu", "Bplus", "SIGNAL", mapping, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Knunu"), "B2Knunu");
    GetNominalNevt(dirname_SIGNAL, "B2Kstarnunu", "Bplus", "SIGNAL", mapping, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Kstarnunu"), "otherwise");
    GetNominalNevt(dirname_SIGNAL, "B2Xsnunu", "Bplus", "SIGNAL", mapping, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Xsnunu"), "B2Xsnunu");
    GetNominalNevt(dirname_SIGNAL, "B02K0nunu", "Bzero", "SIGNAL", mapping, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02K0nunu"), "B02K0nunu");
    GetNominalNevt(dirname_SIGNAL, "B02Kstar0nunu", "Bzero", "SIGNAL", mapping, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Kstar0nunu"), "otherwise");
    GetNominalNevt(dirname_SIGNAL, "B02Xsnunu", "Bzero", "SIGNAL", mapping, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Xsnunu"), "B02Xsnunu");

    double sum = 0;
    for (int i = 0; i < Nbin_MXs_true; i++) {
        for (int j = 0; j < RarityBins; j++) {
            sum = sum + mapping->GetBinContent(i + 1, j + 1);
        }
    }
    mapping->Scale(1 / sum);
    ...
    for (int i = 0; i < Nbin_MXs_true; i++) {
        for (int j = 0; j < RarityBins; j++) {
            printf("%lf, ", mapping->GetBinContent(i + 1, j + 1));
        }
        printf("\n");
    }
    printf("\n");
    for (int i = 0; i < Nbin_MXs_true; i++) {
        for (int j = 0; j < RarityBins; j++) {
            printf("%lf, ", mapping->GetBinError(i + 1, j + 1));
        }
        printf("\n");
    }

...
int main(int argc, char* argv[]) {
    ...
    if (std::string(argv[1]) == std::string("ToyMC")) {  // main ToyMC
        ...
        if (argc == 9) {
            ...
            indicator = std::atoi(argv[6]);
            Toy_iter_num = std::atoi(argv[7]);
            fixed_param = std::string(argv[8]);
        }
        else {
            printf("nuisance Toy MC requires 8 arguments {injected mu_1} {injected mu_2} {injected mu_3} {eps} {indicator} {Num of sample} {fixed param}\n");
            exit(1);
        }
    }
    else if ((std::string(argv[1]) == std::string("nuisanceLOADNevt")) || (std::string(argv[1]) == std::string("nuisanceLOADNevtLOADNuisance")) || (std::string(argv[1]) == std::string("nuisanceLOADNuisanceData"))) { // check the power of nuisance parameters
        if (argc == 6) {
            injected_mu = -1;
            eps = std::atof(argv[2]);
            indicator = std::atoi(argv[3]);
            ...
            printf("nuisanceLOADNevt(LOADNuisance) Toy MC requires 5 arguments {eps} {indicator} {Num of sample} {fixed param}\n");
            exit(1);
        }
    }
    else {
        printf("first argument should be {ToyMC|LinearityTest|nuisance|nuisanceLOADNevt|nuisanceLOADNevtLOADNuisance|nuisanceLOADNuisanceData|ToyMCRC|ToyMCbox}\n");
        exit(1);
    }
    ...
    else if ((std::string(argv[1]) == std::string("nuisanceLOADNevtLOADNuisance")) || (std::string(argv[1]) == std::string("nuisanceLOADNuisanceData"))) {
        OPTIONS* options = (OPTIONS*)malloc(sizeof(OPTIONS));
        Initialize_options(options, fixed_param.c_str());
    ...
    if (std::string(argv[1]) == std::string("ToyMC")) {
        ...
        // fix specific nuisance parameters. Toys are generated by fluctuating free nuisance parameters.
        filesaver.OpenFile(true, &param_names, injected_mu, indicator);
        MyToyMCStudy(w, &param_names, injected_mu, injected_mu_2, injected_mu_3, eps, indicator);
    }
    else if (std::string(argv[1]) == std::string("nuisanceLOADNevt")) {
        // fix specific nuisance parameters. Toys are generated by fluctuating events from Nevt.txt.
        filesaver.OpenFile(true, &param_names, injected_mu, indicator);
        ...
        // load nuisance parameter from nuisance.txt. fix specific nuisance parameters. Toys are generated by fluctuating events from Nevt.txt.
        filesaver.OpenFile(true, &param_names, injected_mu, indicator);
        std::vector<double> Nevts = ReadFittedNevt("Nevts.txt");
        MyToyMCStudyWithNevts(w, &param_names, eps, Nevts, indicator);
    }
    else if (std::string(argv[1]) == std::string("nuisanceLOADNuisanceData")) {
        // load nuisance parameter from nuisance.txt. fix specific nuisance parameters. Toys are generated by fluctuating events from data.
        RooDataSet* data = (RooDataSet*)w->data("obsData");
        filesaver.OpenFile(true, &param_names, injected_mu, indicator);
        MyToyMCStudyDataPoisson(w, &param_names, eps, data, indicator);
    }

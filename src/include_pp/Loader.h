...
typedef struct data {
    ...
    // 189: invppbar, 190: nBplusToXpp, 191: nBzeroToXpp
...
void Loader::GetData(TFile* input_file) {
    ...
    tree_upsilon->SetBranchAddress("daughter__bo1__cm__spdaughterInvariantMass__bo1__cm__sp2__bc__bc", &temp.Upsilon_info[189]);
    if (strcmp(TypeName, "Int_t") == 0) {
        tree_upsilon->SetBranchAddress("nParticlesInList__boB__pl__clXpp__bc", &MakeShiftDoubleToInt[24]); // temp.Upsilon_info[190]
        tree_upsilon->SetBranchAddress("nParticlesInList__boB0__clXpp__bc", &MakeShiftDoubleToInt[25]); // temp.Upsilon_info[191]
    }
    else {
        tree_upsilon->SetBranchAddress("nParticlesInList__boB__pl__clXpp__bc", &temp.Upsilon_info[190]);
        tree_upsilon->SetBranchAddress("nParticlesInList__boB0__clXpp__bc", &temp.Upsilon_info[191]);
    }
    ...
    for (int i_PID = 0; i_PID < N_ProtonID_syst; i_PID++) {
        tree_Bsig->SetBranchAddress(("Bsig_extraInfo_nptruebin" + std::to_string(i_PID)).c_str(), &temp.Bsig_info[763 + i_PID]);
    }
    ...
    for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
        ...
        if (strcmp(TypeName, "Int_t") == 0) {
            ...
            temp.Upsilon_info[190] = static_cast<double>(MakeShiftDoubleToInt[24]);
            temp.Upsilon_info[191] = static_cast<double>(MakeShiftDoubleToInt[25]);
        }
...
void Loader::PrintRootFile(std::string output_name) {
    if (files.size() == current_file && trees_upsilon.size() == current_file && trees_Bsig.size() == current_file && trees_Btag.size() == current_file && trees_Xs.size() == current_file) { // allocate new TFile and TTree
        ...
        tree_upsilon->Branch("daughter__bo1__cm__spdaughterInvariantMass__bo1__cm__sp2__bc__bc", &UpsilonDataToTree[189]);
        tree_upsilon->Branch("nParticlesInList__boB__pl__clXpp__bc", &UpsilonDataToTree[190]);
        tree_upsilon->Branch("nParticlesInList__boB0__clXpp__bc", &UpsilonDataToTree[191]);
        ...
        for (int i_PID = 0; i_PID < N_ProtonID_syst; i_PID++) {
            tree_Bsig->Branch(("Bsig_extraInfo_nptruebin" + std::to_string(i_PID)).c_str(), &BsigDataToTree[763 + i_PID]);
        }
...
void Loader::PrintSeparateRootFile(std::string output_name) {
    ...
    temp_tree_upsilon->Branch("daughter__bo1__cm__spdaughterInvariantMass__bo1__cm__sp2__bc__bc", &temp_UpsilonDataToTree[189]);
    temp_tree_upsilon->Branch("nParticlesInList__boB__pl__clXpp__bc", &temp_UpsilonDataToTree[190]);
    temp_tree_upsilon->Branch("nParticlesInList__boB0__clXpp__bc", &temp_UpsilonDataToTree[191]);
    ...
    for (int i_PID = 0; i_PID < N_ProtonID_syst; i_PID++) {
        temp_tree_Bsig->Branch(("Bsig_extraInfo_nptruebin" + std::to_string(i_PID)).c_str(), &temp_BsigDataToTree[763 + i_PID]);
    }
...
void Loader::ConvertIntoSeparateDataFile(std::string output_name, int flag) {
    ...
    temp_tree->Branch("daughter__bo1__cm__spdaughterInvariantMass__bo1__cm__sp2__bc__bc", &temp_UpsilonDataToTree[189]);
    temp_tree->Branch("nParticlesInList__boB__pl__clXpp__bc", &temp_UpsilonDataToTree[190]);
    temp_tree->Branch("nParticlesInList__boB0__clXpp__bc", &temp_UpsilonDataToTree[191]);
    ...
    for (int i_PID = 0; i_PID < N_ProtonID_syst; i_PID++) {
        temp_tree->Branch(("Bsig_extraInfo_nptruebin" + std::to_string(i_PID)).c_str(), &temp_BsigDataToTree[763 + i_PID]);
    }

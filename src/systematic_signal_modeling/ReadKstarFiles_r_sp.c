...
void LetsFillMKstar(const char* dirname, TH1D* hist) {
...
void ReadKstarFiles_r_sp(){
    const double MKstar_evtpdl = 0.89166;
    const double MKstar_Gamma_evtpdl = 0.01;

    const char* Kstar_dirname = "./Kstarnunu";

    TH1D* hist = new TH1D("MKstar", ";M_{K^{*}} [GeV/c^{2}]; evt", Nentry, MIN_Q2, MAX_Q2);
    TH1D* hist_delta = new TH1D("MKstar_delta", ";M_{K^{*}} [GeV/c^{2}]; evt", Nentry, MIN_Q2, MAX_Q2);

    LetsFillMKstar(Kstar_dirname, hist);
    for (int i = 0; i < Nfine; i++) {
        double temp = MKstar_evtpdl - (MKstar_Gamma_evtpdl / 2.0) + i * MKstar_Gamma_evtpdl / Nfine;
    ...
    hist->Draw("SAME Hist");
    legend->Draw("SAME");
    c_temp->SaveAs("MKstar.png");

    FILE* fp;
    fp = fopen("MKstar_weight.txt","w");

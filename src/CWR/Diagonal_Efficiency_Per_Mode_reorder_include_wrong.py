...
Efficiency_including_wrong = []
Efficiency = np.zeros((30,30))
cross_feed = np.zeros((30,30))

for i in range(0,30):
    Efficiency_including_wrong.append(sum(values[i]) / correction[i])
...
for i in range(0,30):
    for j in range(0,30):
        if Efficiency_including_wrong[j] != 0:
            cross_feed[i][j] = (Efficiency[i][j] / Efficiency_including_wrong[j])
        else:
            cross_feed[i][j] = 0


print(Efficiency_including_wrong[0])
print(values[0][0])
print(values[0][1])

reco_list.reverse()
Efficiency_including_wrong.reverse()

# reorder
index_map = {name: i for i, name in enumerate(reco_list)}
ordered_efficiencies = [Efficiency_including_wrong[index_map[name]] for name in manual_order]

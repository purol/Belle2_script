...
double GetNumEvts(RooWorkspace* w, const char* sample_type) {
    ...
    else if (strcmp(sample_type, "MIX_MX1") == 0) indices.push_back(4);
    else if (strcmp(sample_type, "UUBAR_MX1") == 0) indices.push_back(5);
    else if (strcmp(sample_type, "DDBAR_MX1") == 0) indices.push_back(6);
    else if (strcmp(sample_type, "SSBAR_MX1") == 0) indices.push_back(7);
    else if (strcmp(sample_type, "CHARM_MX1") == 0) indices.push_back(8);
    else {
        printf("[ERROR] unexpected sample type!\n");
        exit(1);
    }
...
void GetPlotTemplate(RooWorkspace* w, RooDataSet* data = nullptr, const char * plot_name = "hist_data_plot.png") {
    ...
    pt_lumi->SetTextSize(0.035); pt_lumi->SetFillStyle(0); pt_lumi->SetLineWidth(0); pt_lumi->SetTextAlign(11); pt_lumi->AddText("#int L dt = 364.5 fb^{-1}"); pt_lumi->Draw();

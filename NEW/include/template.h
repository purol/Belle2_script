...
double FillTemplate(TH1D* hist, double FBDToutput, double total_weight, double MXs) {
	if ((MXs > 0.0) && (MXs < 0.6)) { // MXs1
		if ((FBDToutput >= BinMIN_MX1) && (FBDToutput <= BinMAX_MX1)) {
			...
			double binindex = std::floor((FBDToutput - BinMIN_MX1) / binwidth); // start from 0. global bin index
		...
		if ((FBDToutput >= BinMIN_MX2) && (FBDToutput <= BinMAX_MX2)) {
			...
			double binindex = RarityBins_MX1 + std::floor((FBDToutput - BinMIN_MX2) / binwidth); // start from RarityBins_MX1. global bin index
		...
		if ((FBDToutput >= BinMIN_MX3) && (FBDToutput <= BinMAX_MX3)) {
			...
			double binindex = RarityBins_MX1 + RarityBins_MX2 + std::floor((FBDToutput - BinMIN_MX3) / binwidth); // start from RarityBins_MX1 + RarityBins_MX2. global bin index
...
double ReturnBinIndex(double FBDToutput, double MXs) {
	if ((MXs > 0.0) && (MXs < 0.6)) { // MXs1
		if ((FBDToutput >= BinMIN_MX1) && (FBDToutput <= BinMAX_MX1)) {
			...
			return (binindex + 0.5); // add 0.5 to avoid unexpected rounding error
		}
		else {
			printf("FBDT output is out of range!\n");
			exit(1);

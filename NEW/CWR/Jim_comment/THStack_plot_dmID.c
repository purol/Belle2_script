...
void THStack_plot_dmID() {
    ...
    LetsCountMC(MC_MIX_validation_dirname, "MIX");
    LetsCountMC(MC_UUBAR_validation_dirname, "UUBAR");
    LetsCountMC(MC_DDBAR_validation_dirname, "DDBAR");
    LetsCountMC(MC_SSBAR_validation_dirname, "SSBAR");
    LetsCountMC(MC_CHARM_validation_dirname, "CHARM");

    LetsCountdata(data_dirname);
    ...
    /*
    LetsFillMC(MC_CHG_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "CHG");
    LetsFillMC(MC_MIX_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "MIX");
    LetsFillMC_correction(MC_UUBAR_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "UUBAR", NormFactor_UUBAR);
    LetsFillMC_correction(MC_DDBAR_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "DDBAR", NormFactor_DDBAR);
    LetsFillMC_correction(MC_SSBAR_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "SSBAR", NormFactor_SSBAR);
    LetsFillMC_correction(MC_CHARM_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "CHARM", NormFactor_CHARM);
    */
    LetsFillMC(MC_CHG_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "CHG");
    LetsFillMC(MC_MIX_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "MIX");
    LetsFillMC(MC_UUBAR_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "UUBAR");
    LetsFillMC(MC_DDBAR_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "DDBAR");
    LetsFillMC(MC_SSBAR_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "SSBAR");
    LetsFillMC(MC_CHARM_validation_dirname, variable_names, branch_names, MC_values, &MC_numbering, &weights, "CHARM");
    
    LetsFilldata(data_dirname, variable_names, branch_names, data_values);

    // sort variables
    for (int k = 0; k < (int)MC_numbering.size(); k++) {
        if (MC_numbering.at(k) == static_cast<int>(MCsample::CHG)) {
            for (int l = 0; l < (int)variable_names.size(); l++) charged_values[l].push_back(MC_values[l].at(k));
            charged_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::MIX)) {
            for (int l = 0; l < (int)variable_names.size(); l++) mixed_values[l].push_back(MC_values[l].at(k));
            mixed_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::UUBAR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) uubar_values[l].push_back(MC_values[l].at(k));
            uubar_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::DDBAR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ddbar_values[l].push_back(MC_values[l].at(k));
            ddbar_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::SSBAR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ssbar_values[l].push_back(MC_values[l].at(k));
            ssbar_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::CHARM)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ccbar_values[l].push_back(MC_values[l].at(k));
            ccbar_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::TAU)) {
            for (int l = 0; l < (int)variable_names.size(); l++) taupair_values[l].push_back(MC_values[l].at(k));
            taupair_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::MUMU)) {
            for (int l = 0; l < (int)variable_names.size(); l++) mumu_values[l].push_back(MC_values[l].at(k));
            mumu_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::GG)) {
            for (int l = 0; l < (int)variable_names.size(); l++) gg_values[l].push_back(MC_values[l].at(k));
            gg_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::EE)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ee_values[l].push_back(MC_values[l].at(k));
            ee_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::EEEE)) {
            for (int l = 0; l < (int)variable_names.size(); l++) eeee_values[l].push_back(MC_values[l].at(k));
            eeee_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::EEMUMU)) {
            for (int l = 0; l < (int)variable_names.size(); l++) eemumu_values[l].push_back(MC_values[l].at(k));
            eemumu_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::LLXX)) {
            for (int l = 0; l < (int)variable_names.size(); l++) llXX_values[l].push_back(MC_values[l].at(k));
            llXX_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == static_cast<int>(MCsample::HHISR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) hhISR_values[l].push_back(MC_values[l].at(k));
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // draw
        ...
        if((variable_names.at(k).find("MVA") != std::string::npos) && (MC_values[k].size() > 10000)) pad1->SetLogy(1);
        else pad1->SetLogy(0);
    ...
    if (NormalizeAtEachMXs == false) {
        double MC_sum = 0;
        for (int i = 0; i < (int)MC_values[0].size(); i++) MC_sum = MC_sum + weights.at(i);
        printf("data num: %ld\n", data_values[0].size());

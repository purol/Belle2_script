...
double** GetPDFs(const char* dirname, const char* included_string, const char* type, const char* sample, double weight_var = 1.0, std::string CorrectionType = "otherwise") { // get nominal PDF with appropriate correction
    /*
...
void GetSwaveFraction_advanced(){
    ...
    LetsFillMK0star(K0star_dirname, &list, "B02Kstar");
    ...
    LetsFillMK0star(Kplusstar_dirname, &list, "B2Kstar");
    for (int i = 0; i < list.size(); i++) {
        int index_qsq = -1;
        int index_mass = -1;
        ...
        N_Kplusstar[index_qsq][index_mass] = N_Kplusstar[index_qsq][index_mass] + Scale_K0star_here;
    }

    LetsFillMK0star(kappa0_dirname, &list, "B02Kappa");
    for (int i = 0; i < list.size(); i++) {
        int index_qsq = -1;
        int index_mass = -1;
    ...
    LetsFillMK0star(kappaplus_dirname, &list, "B2Kappa");
    for (int i = 0; i < list.size(); i++) {
        int index_qsq = -1;
        int index_mass = -1;
    ...
    double** N_Kstar_after = GetPDFs(dir_Ntuple, "B2Kstar", "Bplus", "SIGNAL", Scale_Kplusstar_here, "otherwise");
    double** N_K0star_after = GetPDFs(dir_Ntuple, "B02Kstar", "Bzero", "SIGNAL", Scale_K0star_here, "otherwise");
    double** N_kappa_after = GetPDFs(dir_Ntuple, "B2Kappa", "Bplus", "SIGNAL", 1.0, "otherwise");
    double** N_kappa0_after = GetPDFs(dir_Ntuple, "B02Kappa", "Bzero", "SIGNAL", 1.0, "otherwise");

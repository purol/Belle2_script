            ...
            if (pdftype == PDFtype::nominal) {}
                ...
                if (strcmp(sample, "SIGNAL") == 0) { // it works only when the sample is signal
                    if (std::abs(Btag_isSignal - 1.0) < MyEPSILON) { // it is applied only when Btag is correct
                        ...
                        if (strcmp(type, "Bplus") == 0) {
                            if (std::abs(Btag_ID - 0.0) < MyEPSILON) New_Correction_FEI = 0.878850;
                            else if (std::abs(Btag_ID - 1.0) < MyEPSILON) New_Correction_FEI = 0.878850;
                            else if (std::abs(Btag_ID - 3.0) < MyEPSILON) New_Correction_FEI = 0.878850;
                            else if (std::abs(Btag_ID - 4.0) < MyEPSILON) New_Correction_FEI = 0.878850;
                            else if (std::abs(Btag_ID - 15.0) < MyEPSILON) New_Correction_FEI = 0.578703;
                            else if (std::abs(Btag_ID - 16.0) < MyEPSILON) New_Correction_FEI = 0.578703;
                            else if (std::abs(Btag_ID - 18.0) < MyEPSILON) New_Correction_FEI = 0.578703;
                            else if (std::abs(Btag_ID - 19.0) < MyEPSILON) New_Correction_FEI = 0.578703;
                            else if (std::abs(Btag_ID - 23.0) < MyEPSILON) New_Correction_FEI = 0.227201;
                            else if (std::abs(Btag_ID - 24.0) < MyEPSILON) New_Correction_FEI = 0.227201;
                            else if (std::abs(Btag_ID - 30.0) < MyEPSILON) New_Correction_FEI = 0.227201;
                            else New_Correction_FEI = 0.227201;
                        }
                        else if (strcmp(type, "Bzero") == 0) New_Correction_FEI = 0.954938;
...
void ReadSignalModelingFile(std::string input_path) {
    ...
    fp = fopen((input_path + "/Xsu_Htransition_weight.txt").c_str(), "r");
    fscanf(fp, "%d\n", &Nentry);
    ...
    fp = fopen((input_path + "/Xsu_Ltransition_weight.txt").c_str(), "r");
    fscanf(fp, "%d\n", &Nentry);
    if (Nentry != RarityBins) {
        printf("[ReadSignalModelingFile] Nentry and RarityBins are not the same!\n");
        exit(1);
    ...
    fp = fopen((input_path + "/Xsd_Htransition_weight.txt").c_str(), "r");
    fscanf(fp, "%d\n", &Nentry);
    if (Nentry != RarityBins) {
        printf("[ReadSignalModelingFile] Nentry and RarityBins are not the same!\n");
        exit(1);
    ...
    fp = fopen((input_path + "/Xsd_Ltransition_weight.txt").c_str(), "r");
    fscanf(fp, "%d\n", &Nentry);
    if (Nentry != RarityBins) {
        printf("[ReadSignalModelingFile] Nentry and RarityBins are not the same!\n");
        exit(1);
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    const char* MC_dirname_SIGNAL = strdup(("/home/belle2/junewoo/storage_ghi/Analysis/KumoiRD/SIGNAL_analysis/validation_" + std::string(argv[1]) + "/final_output_root_after_MVA_Application_after_cut/Merge").c_str());

    const char* MC_dirname_CHG = strdup(("/home/belle2/junewoo/storage_ghi/Analysis/KumoiRD/CHG_analysis/validation_" + std::string(argv[1]) + "/final_output_root_after_MVA_Application_after_cut/Merge").c_str());
    const char* MC_dirname_MIX = strdup(("/home/belle2/junewoo/storage_ghi/Analysis/KumoiRD/MIX_analysis/validation_" + std::string(argv[1]) + "/final_output_root_after_MVA_Application_after_cut/Merge").c_str());
    const char* MC_dirname_UUBAR = strdup(("/home/belle2/junewoo/storage_ghi/Analysis/KumoiRD/UUBAR_analysis/validation_" + std::string(argv[1]) + "/final_output_root_after_MVA_Application_after_cut/Merge").c_str());
    const char* MC_dirname_DDBAR = strdup(("/home/belle2/junewoo/storage_ghi/Analysis/KumoiRD/DDBAR_analysis/validation_" + std::string(argv[1]) + "/final_output_root_after_MVA_Application_after_cut/Merge").c_str());
    const char* MC_dirname_SSBAR = strdup(("/home/belle2/junewoo/storage_ghi/Analysis/KumoiRD/SSBAR_analysis/validation_" + std::string(argv[1]) + "/final_output_root_after_MVA_Application_after_cut/Merge").c_str());
    const char* MC_dirname_CHARM = strdup(("/home/belle2/junewoo/storage_ghi/Analysis/KumoiRD/CHARM_analysis/validation_" + std::string(argv[1]) + "/final_output_root_after_MVA_Application_after_cut/Merge").c_str());
 
    // for FEI
    const char* FEI_correlated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/FEI_selected.txt").c_str());
    const char* FEI_uncorrelated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/FEI_cov_remain_truncated.txt").c_str());

    // for PID
    const char* KID_correlated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/KID_selected.txt").c_str());
    const char* PID_correlated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/PID_selected.txt").c_str());
    const char* KID_uncorrelated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/KID_cov_remain_truncated.txt").c_str());
    const char* PID_uncorrelated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/PID_cov_remain_truncated.txt").c_str());

    // for BB BR
    const char* BR_correlated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/BR_selected.txt").c_str());
    const char* BR_uncorrelated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/BR_cov_remain_truncated.txt").c_str());

    // for pi0
    const char* pi0_correlated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/pi0_selected.txt").c_str());
    const char* pi0_uncorrelated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/pi0_cov_remain_truncated.txt").c_str());

    // for multiplicity
    const char* multiplicity_correlated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/multiplicity_selected.txt").c_str());
    const char* multiplicity_uncorrelated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/multiplicity_cov_remain_truncated.txt").c_str());

    // for fragmentation
    const char* Fragmentation_correlated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/Fragmentation_selected.txt").c_str());
    const char* Fragmentation_uncorrelated_info = strdup(("./KumoiRD_" + std::string(argv[1]) + "/set1/Fragmentation_cov_remain_truncated.txt").c_str());

    // for additional relative uncertainty
    const char* relative_uncertainty_file = "./KumoiRD_v009/set1/dataMCratio_sideband.txt";
    ...
    /* ====================================== */
    // Save histograms
    TFile* file = new TFile((std::string(argv[2]) + "/PDFandDATA_" + recon_MXs_bin + ".root").c_str(), "RECREATE");

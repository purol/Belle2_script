...
// gaussian parameter for B->XKLKL, B->Xnn, and B->(D->XKL) + anything
double alpha_XKLKL = 1.0;
double alpha_Xnn = 1.0;
double alpha_BtoDtoXKL = 1.0;
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
            ...
            if ((strcmp(sample, "CHG") == 0) || (strcmp(sample, "MIX") == 0) || (strcmp(sample, "SIGNAL") == 0)) {
                ...
                Correction_BtoDtoXKL = Correction_BtoDtoXKL * (1 + corrector_BtoDtoXKL.GetRelativeUncertainty(nDptoXKL + nD0toXKL) * (alpha_BtoDtoXKL - 1.0));
            }
...
void FluctuateBBBR() {
    ...
    std::normal_distribution<double> extra_BR_distribution(1.0, 1.0);

    alpha_XKLKL = extra_BR_distribution(generator);
    alpha_Xnn = extra_BR_distribution(generator);
    alpha_BtoDtoXKL = extra_BR_distribution(generator);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < NToys; i++) {
        ...
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B2Xsnunu", temp_hist, "Bplus", "SIGNAL", Nevt_fluc, i, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Xsnunu"), "B2Xsnunu");
        temp_hist->Reset();
        ...
        temp_hist->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B02Xsnunu", temp_hist, "Bzero", "SIGNAL", Nevt_fluc, i, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Xsnunu"), "B02Xsnunu");
        temp_hist->Reset();

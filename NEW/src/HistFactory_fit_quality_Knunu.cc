...
std::vector<double> MySetParamsForToy(RooWorkspace* w, std::vector<std::string>* names, double injected_mu) {
    ...
    for (int i = 0; i < RarityBins_MX1; i++) Nevt.push_back(0.0);
    ...
    for (int j = 0; j < RarityBins_MX1; j++) {
        ...
        if (data->weight() > MyEPSILON) {
            if (extended) {
                ...
                genData->add(RooArgSet(*x_val_MXs1, *channelCat), (int)floor(Nevts.at(j) + 0.5));
            }
        }
...
void MyToyMCRCStudy(RooWorkspace* w, std::vector<std::string>* names, double eps, int indicator = 0) {
    ...
    for (int i = 0; i < Toy_iter_num; i++) { // Do Toy MC study
        ...
        for (int j = 0; j < RarityBins_MX1; j++) {
            ...
            x_val_MXs1->setVal(0.5 + j);
            channelCat->setLabel("channel_MXs1");
            genData->add(RooArgSet(*x_val_MXs1, *channelCat), Nevts.at(j) + PoissonError.at(j));
        }
...
void FitToData(RooWorkspace* w, double eps) {
    ...
    // fit
    //RooFitResult* fitres = model->fitTo(*data, RooFit::Minimizer("Minuit2"), RooFit::Extended(false), RooFit::Minos(RooArgSet(*w->var("mu_MXs1"))), RooFit::SumW2Error(false), Save());
    RooAbsReal* nll;
...
int main(int argc, char* argv[]) {
    ...
    if ((std::string(argv[1]) == std::string("ToyMC")) || (std::string(argv[1]) == std::string("ToyMCRC")) || (std::string(argv[1]) == std::string("ToyMCbox"))) {  // main ToyMC|ToyMCRC
        if (argc == 5) {
            injected_mu = -1;
            eps = std::atof(argv[2]);

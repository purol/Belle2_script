...
# Kstarff
file_Kstarff_corr_all = "Kstarff_corr_all.txt"
file_Kstarff_corr_partial = "Kstarff_corr_partial.txt"

Kstarff_corr_all = np.loadtxt(file_Kstarff_corr_all, dtype=float)
Kstarff_corr_partial = np.loadtxt(file_Kstarff_corr_partial, dtype=float)

ar = np.array(Kstarff_corr_all)
ax = sn.heatmap(ar, cmap="viridis", xticklabels=[], yticklabels=[], vmin=-1.0, vmax=1.0)
ax.tick_params(left=False, bottom=False)

labels = ["CHG", "MIX", "Signal"]
ticks = [ i * Nbins for i in range(4)]
...
#plt.show()
plt.savefig('Kstarff_corr_all.png')

plt.clf()

ar_p = np.array(Kstarff_corr_partial)
...
#plt.show()
plt.savefig('Kstarff_corr_partial.png')

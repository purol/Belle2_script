...
% FEI
FEIfileID = fopen(fullfile(base_dir, 'FEI_toys.txt'),'r');
...
FEIfileID_w = fopen(fullfile(base_dir, 'FEI_cov_all.txt'),'w');
...
FEIfileID_w = fopen(fullfile(base_dir, 'FEI_cov_all_truncated.txt'),'w');
for i = 1:N_total_bins
...
FEIfileID_w = fopen(fullfile(base_dir, 'FEI_corr_all.txt'),'w');
for i = 1:N_total_bins
    for j = 1:N_total_bins
...
FEIfileID_w = fopen(fullfile(base_dir, 'FEI_selected.txt'),'w');
for j = 1:N_selected_vectors
    fprintf(FEIfileID_w,"%1.15f\n", sqrt(FEIlatent(j)));
    for i = 1:N_total_bins
...
FEIfileID_w = fopen(fullfile(base_dir, 'FEI_cov_partial.txt'),'w');
for i = 1:N_total_bins
    for j = 1:N_total_bins
...
FEIfileID_w = fopen(fullfile(base_dir, 'FEI_corr_partial.txt'),'w');
for i = 1:N_total_bins
    for j = 1:N_total_bins
...
FEIfileID_w = fopen(fullfile(base_dir, 'FEI_cov_remain_truncated.txt'),'w');
for i = 1:N_total_bins
    fprintf(FEIfileID_w,"%1.15f\n",FEIremaincov(i,i));
end
fclose(FEIfileID_w);

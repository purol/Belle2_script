...
% pi0 correction
pi0fileID = fopen(fullfile(base_dir, 'pi0_toys.txt'),'r');
...
pi0fileID_w = fopen(fullfile(base_dir, 'pi0_cov_all.txt'),'w');
...
pi0fileID_w = fopen(fullfile(base_dir, 'pi0_cov_all_truncated.txt'),'w');
for i = 1:N_total_bins
...
pi0fileID_w = fopen(fullfile(base_dir, 'pi0_corr_all.txt'),'w');
for i = 1:N_total_bins
    for j = 1:N_total_bins
...
pi0fileID_w = fopen(fullfile(base_dir, 'pi0_selected.txt'),'w');
for j = 1:N_selected_vectors
    fprintf(pi0fileID_w,"%1.15f\n", sqrt(pi0latent(j)));
    for i = 1:N_total_bins
...
pi0fileID_w = fopen(fullfile(base_dir, 'pi0_cov_partial.txt'),'w');
for i = 1:N_total_bins
    for j = 1:N_total_bins
...
pi0fileID_w = fopen(fullfile(base_dir, 'pi0_corr_partial.txt'),'w');
for i = 1:N_total_bins
    for j = 1:N_total_bins
...
pi0fileID_w = fopen(fullfile(base_dir, 'pi0_cov_remain_truncated.txt'),'w');
for i = 1:N_total_bins
    fprintf(pi0fileID_w,"%1.15f\n",pi0remaincov(i,i));
end
fclose(pi0fileID_w);

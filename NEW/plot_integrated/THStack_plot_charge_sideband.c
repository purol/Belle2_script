...
void THStack_plot_charge_sideband() {
    ...
    std::vector<double>* ChargeSideband_MC_values = new std::vector<double>[Nvar_num];
    ...
    std::vector<int> ChargeSideband_MC_numbering;

    std::vector<double>* ChargeSideband_data_values = new std::vector<double>[Nvar_num];
    ...
    // sort variables
    for (int k = 0; k < (int)ChargeSideband_MC_numbering.size(); k++) {
        if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::CHG)) {
            for (int l = 0; l < (int)variable_names.size(); l++) charged_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            charged_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::MIX)) {
            for (int l = 0; l < (int)variable_names.size(); l++) mixed_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            mixed_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::UUBAR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) uubar_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            uubar_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::DDBAR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ddbar_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            ddbar_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::SSBAR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ssbar_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            ssbar_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::CHARM)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ccbar_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            ccbar_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::TAU)) {
            for (int l = 0; l < (int)variable_names.size(); l++) taupair_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            taupair_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::MUMU)) {
            for (int l = 0; l < (int)variable_names.size(); l++) mumu_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            mumu_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::GG)) {
            for (int l = 0; l < (int)variable_names.size(); l++) gg_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            gg_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::EE)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ee_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            ee_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::EEEE)) {
            for (int l = 0; l < (int)variable_names.size(); l++) eeee_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            eeee_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::EEMUMU)) {
            for (int l = 0; l < (int)variable_names.size(); l++) eemumu_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            eemumu_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::LLXX)) {
            for (int l = 0; l < (int)variable_names.size(); l++) llXX_values[l].push_back(ChargeSideband_MC_values[l].at(k));
            llXX_weights.push_back(weights.at(k));
        }
        else if (ChargeSideband_MC_numbering.at(k) == static_cast<int>(MCsample::HHISR)) {
            for (int l = 0; l < (int)variable_names.size(); l++) hhISR_values[l].push_back(ChargeSideband_MC_values[l].at(k));
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // malloc TH1D
        ...
        temp_v.insert(temp_v.end(), ChargeSideband_data_values[k].begin(), ChargeSideband_data_values[k].end());
    ...
    for (int i = 0; i < (int)ChargeSideband_MC_values[index].size(); i++) ChargeSideband_MC_values[index].at(i) = log10l(ChargeSideband_MC_values[index].at(i));
    for (int i = 0; i < (int)ChargeSideband_data_values[index].size(); i++) ChargeSideband_data_values[index].at(i) = log10l(ChargeSideband_data_values[index].at(i));
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // fill
        ...
        for (int i = 0; i < (int)ChargeSideband_MC_values[k].size(); i++) stat_error_hist[k]->Fill(ChargeSideband_MC_values[k].at(i), weights.at(i));
        for (int i = 0; i < (int)ChargeSideband_data_values[k].size(); i++) data_hist[k]->Fill(ChargeSideband_data_values[k].at(i));

        if (variable_names.at(k) == "nROE_ParticlesInList__bopi0__clmyneutralPion__bc") {
            for (int i = 0; i < (int)ChargeSideband_MC_values[k].size(); i++) MC_one_bin->Fill(ChargeSideband_MC_values[k].at(i), weights.at(i));
            for (int i = 0; i < (int)ChargeSideband_data_values[k].size(); i++) data_one_bin->Fill(ChargeSideband_data_values[k].at(i));
        }
    }
    ...
    printf("data: %d\n", (int)ChargeSideband_data_values[0].size());
    ...
    double MC_sum = 0;
    for (int i = 0; i < (int)ChargeSideband_MC_values[0].size(); i++) MC_sum = MC_sum + weights.at(i);
    printf("data num: %ld\n", ChargeSideband_data_values[0].size());
    ...
    // free
    delete[] ChargeSideband_MC_values;
    ...
    delete[]  ChargeSideband_data_values;

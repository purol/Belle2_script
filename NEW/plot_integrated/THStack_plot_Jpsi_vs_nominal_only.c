...
IT SHOULD BE NOT BE USED AFTER PRL REVIEW
============================================================================================== */
...
void GetSignalMC(const char* Nominal_MC_SIGNAL_validation_dirname, TH1D*** signal_hist, std::vector<std::string> variable_names, std::vector<std::string> branch_names) {
    ...
    for (int k = 0; k < (int)Nominal_MC_numbering.size(); k++) {
        if (Nominal_MC_numbering.at(k) == static_cast<int>(MCsample::SIGNAL)) {
    ...
    *signal_hist = (TH1D**)malloc(sizeof(TH1D*) * Nvar_num);
...
void THStack_plot_Jpsi_vs_nominal() {

    NormalizeAtEachMXs = false;
    ...
    double N_Jpsi_MC = MC_one_bin->GetBinContent(1);
    ...
    TH1D** signal_hist_nominal;
    GetSignalMC(Nominal_MC_SIGNAL_validation_dirname, &signal_hist_nominal, variable_names, branch_names);

    double N_Xs_MC = MC_one_bin->GetBinContent(1);

    if (NormalizeAtEachMXs == false) {
        Ratio_one_bin->SetBinContent(1, N_Jpsi_MC / N_Xs_MC);
    }
    else {
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // draw
        ...
        if (NormalizeAtEachMXs == false) {
            ...
            signal_hist_nominal[k]->Scale(CAL);
        }
        ...
        Ratio_hist[k]->Divide(signal_hist_nominal[k], stat_error_hist[k]);

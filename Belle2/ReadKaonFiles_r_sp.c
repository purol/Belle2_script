...
void Loader::GetData(TFile* input_file) {
    TTree* tree_Kaon = (TTree*)input_file->Get("Kaon");

    Data temp = { 0 };

    // get event_info
    tree_Kaon->SetBranchAddress("__experiment__", &temp.__experiment__);
    tree_Kaon->SetBranchAddress("__run__", &temp.__run__);
    tree_Kaon->SetBranchAddress("__event__", &temp.__event__);
    tree_Kaon->SetBranchAddress("__candidate__", &temp.__candidate__);
    tree_Kaon->SetBranchAddress("__ncandidates__", &temp.__ncandidates__);

    // get Upsilon_info
    tree_Kaon->SetBranchAddress("kaonID", &temp.variables[0]);
    tree_Kaon->SetBranchAddress("nCDCHits", &temp.variables[1]);
    tree_Kaon->SetBranchAddress("dr", &temp.variables[2]);
    tree_Kaon->SetBranchAddress("dz", &temp.variables[3]);
    tree_Kaon->SetBranchAddress("M", &temp.variables[4]);
    tree_Kaon->SetBranchAddress("dM", &temp.variables[5]);

    printf("%lld entries...\n", tree_Kaon->GetEntries());
    for (unsigned int j = 0; j < tree_Kaon->GetEntries(); j++) { // Fill
...
void ReadKaonFiles_r_sp(){
    ...
    for(unsigned int i = 0; i<names.size(); i++){
        ...
        loader.DrawTH1F("kaonID", "kaonID of kaon candidates;kaonID;evt", 100, 0, 1, Loader::variables, 0);
        loader.DrawTH1F("nCDCHits", "nCDCHits of kaon candidates;nCDCHits;evt", 100, 0, 60, Loader::variables, 1);
        loader.DrawTH1F("dr", "dr of kaon candidates;dr [cm];evt", 100, 0, 1.5, Loader::variables, 2);
        loader.DrawTH1F("dz", "dz of kaon candidates;dz [cm];evt", 100, -4, 4, Loader::variables, 3);
        loader.Cut(Loader::variables, 0, Loader::larger_than, 0.6);
        ...
        loader.PrintInformation(std::string("========== kaonID > 0.6 & nCDCHits > 20 & dr < 0.5 & abs(dz) < 2.0 =========="));
        loader.DrawTH1F("M_after_cuts", "M of kaon candidates;M [GeV];evt", 100, 0.492, 0.493, Loader::variables, 4);
    }
    loader.End();
}

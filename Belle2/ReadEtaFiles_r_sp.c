...
void Loader::GetData(TFile* input_file) {
    TTree* tree_Eta = (TTree*)input_file->Get("Eta");

    Data temp = { 0 };

    // get event_info
    tree_Eta->SetBranchAddress("__experiment__", &temp.__experiment__);
    tree_Eta->SetBranchAddress("__run__", &temp.__run__);
    tree_Eta->SetBranchAddress("__event__", &temp.__event__);
    tree_Eta->SetBranchAddress("__candidate__", &temp.__candidate__);
    tree_Eta->SetBranchAddress("__ncandidates__", &temp.__ncandidates__);

    // get Upsilon_info
    tree_Eta->SetBranchAddress("M", &temp.variables[0]);
    tree_Eta->SetBranchAddress("p", &temp.variables[1]);
    tree_Eta->SetBranchAddress("useCMSFrame__bop__bc", &temp.variables[2]);
    tree_Eta->SetBranchAddress("dM", &temp.variables[3]);
    tree_Eta->SetBranchAddress("cosHelicityAngleMomentum", &temp.variables[4]);

    printf("%lld entries...\n", tree_Eta->GetEntries());
    for (unsigned int j = 0; j < tree_Eta->GetEntries(); j++) { // Fill
        tree_Eta->GetEntry(j);
        TotalData.push(temp);
    }
    input_file->Close();
...
void ReadEtaFiles_r_sp(){
    ...
    for(unsigned int i = 0; i<names.size(); i++){
        ...
        loader.DrawTH1F("M", "mass of #eta candidates;mass [GeV];evt", 100, 0, 1, Loader::variables, 0);
        loader.DrawTH1F("p", "momentum of #eta candidates;p [GeV];evt", 100, 0, 1, Loader::variables, 1);
        loader.DrawTH1F("p_CMS", "momentum of #eta candidates in CMS;#Del#phi [rad];evt", 100, 0, 1, Loader::variables, 2);
        loader.DrawTH1F("dM", "dM of #eta candidates;dM [rad];evt", 100, -1, 1, Loader::variables, 3);
        loader.DrawTH1F("coshelicityangle", "cos#theta_{heli} of #eta candidates;cos#theta_{heli};evt", 100, -1, 1, Loader::variables, 4);

        loader.Cut(Loader::variables, 2, Loader::smaller_than, 0.5);
        loader.Cut(Loader::variables, 4, Loader::smaller_than, 0.8);
        loader.Cut(Loader::variables, 4, Loader::larger_than, -0.8);

        loader.DrawTH1F("M_after_other_cuts", "mass of #eta candidates;mass [GeV];evt", 100, 0, 1, Loader::variables, 0);
        loader.DrawTH1F("dM_after_other_cuts", "dM of #eta candidates;dM [rad];evt", 100, -1, 1, Loader::variables, 3);
        loader.Cut(Loader::variables, 0, Loader::smaller_than, 0.570);
        loader.Cut(Loader::variables, 0, Loader::larger_than, 0.515);
    }
    loader.End();
}

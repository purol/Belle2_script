...
void FillVariables(const char* filename, std::vector<float> input_vars[Nvar], std::vector<bool>* IsSignal, std::vector<float>* weight, bool tempissignal, float weight_N = 1.0, std::string CorrectionType = "otherwise") {
    ...
    */
    ...
    for (unsigned int j = 0; j < tree_data->GetEntries(); j++) { // Fill
        ...
        if (Xs_bin == 1) {
            if ((Mxs > 0.0) && (Mxs < 0.6)) {} // no problem
            else continue;
        }
        else if (Xs_bin == 2) {
            if ((Mxs > 0.6) && (Mxs < 1.0)) {} // no problem
            else continue;
        }
        else if (Xs_bin == 3) {
            if ((Mxs > 1.0) && (Mxs < 2.0)) {} // no problem
            else continue;
        }
        else {
            printf("unexpected Mxs bin and Mxs!\n");
            exit(1);
        }
        Nevt++;
...
int main(int argc, char* argv[])
{
    ...
    * argv[9]: Xs bin (1: 0 < MXs < 0.6, 2: 0.6 < MXs < 1.0, 3: 1.0 < MXs < 2.0)
    */
    ...
    FILE* fp;
    fp = fopen(("/home/belle2/junewoo/storage_b1/bsub/Analysis/GridSearch/out/Result_" + std::string(argv[9]) + "_" + std::string(argv[1]) + "_" + std::string(argv[2]) + "_" + std::string(argv[3]) + "_" + std::string(argv[4]) + "_" + std::string(argv[5])).c_str(), "w");
    ...
    // save model
    std::fstream out_stream(("/home/belle2/junewoo/storage_b1/bsub/Analysis/GridSearch/out/classifier_" + std::string(argv[9]) + "_" + std::string(argv[1]) + "_" + std::string(argv[2]) + "_" + std::string(argv[3]) + "_" + std::string(argv[4]) + "_" + std::string(argv[5]) + ".weightfile").c_str(), std::ios_base::out | std::ios_base::trunc);

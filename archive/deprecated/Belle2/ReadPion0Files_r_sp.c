...
void Loader::GetData(TFile* input_file) {
    TTree* tree_Pion0 = (TTree*)input_file->Get("Pion0");

    Data temp = { 0 };

    // get event_info
    tree_Pion0->SetBranchAddress("__experiment__", &temp.__experiment__);
    tree_Pion0->SetBranchAddress("__run__", &temp.__run__);
    tree_Pion0->SetBranchAddress("__event__", &temp.__event__);
    tree_Pion0->SetBranchAddress("__candidate__", &temp.__candidate__);
    tree_Pion0->SetBranchAddress("__ncandidates__", &temp.__ncandidates__);

    // get Upsilon_info
    tree_Pion0->SetBranchAddress("M", &temp.variables[0]);
    tree_Pion0->SetBranchAddress("p", &temp.variables[1]);
    tree_Pion0->SetBranchAddress("daughterDiffOfPhi__bo0__cm1__bc", &temp.variables[2]);
    tree_Pion0->SetBranchAddress("daughterAngle__bo0__cm1__bc", &temp.variables[3]);

    printf("%lld entries...\n", tree_Pion0->GetEntries());
    for (unsigned int j = 0; j < tree_Pion0->GetEntries(); j++) { // Fill
        tree_Pion0->GetEntry(j);
        TotalData.push(temp);
    }
    input_file->Close();
}
...
void ReadPion0Files_r_sp(){
    ...
    for(unsigned int i = 0; i<names.size(); i++){
        ...
        loader.DrawTH1F("M", "mass of #pi^{0} candidates;mass [GeV];evt", 100, 0.08, 0.18, Loader::variables, 0);
        loader.DrawTH1F("p", "momentum of #pi^{0} candidates;p [GeV];evt", 100, 0, 2, Loader::variables, 1);
        loader.DrawTH1F("delphi", "difference of #phi between two gammas from #pi^{0};#Del#phi [rad];evt", 100, -3.2, 3.2, Loader::variables, 2);
        loader.DrawTH1F("delalpha", "difference of angle between two gammas from #pi^{0};#Del#alpha [rad];evt", 100, 0, 3.2, Loader::variables, 3);

        loader.Cut(Loader::variables, 1, Loader::larger_than, 0.4);
        loader.Cut(Loader::variables, 2, Loader::larger_than, -1.5);
        loader.Cut(Loader::variables, 2, Loader::smaller_than, 1.5);
        loader.Cut(Loader::variables, 3, Loader::smaller_than, 1.4);
        loader.DrawTH1F("M_after_other_cuts", "mass of #pi^{0} candidates;mass [GeV];evt", 100, 0.08, 0.18, Loader::variables, 0);
        loader.Cut(Loader::variables, 0, Loader::smaller_than, 0.145);
        loader.Cut(Loader::variables, 0, Loader::larger_than, 0.12);
    }
    loader.End();
}

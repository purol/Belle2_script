...
int main(int argc, char* argv[])
{
    ...
    }
    {
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((CHG_input_train + std::string("/") + names.at(i)).c_str(), input_vars, &IsSignal, &weight, false, Scale_CHG_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((MIX_input_train + std::string("/") + names.at(i)).c_str(), input_vars, &IsSignal, &weight, false, Scale_MIX_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((UUBAR_input_train + std::string("/") + names.at(i)).c_str(), input_vars, &IsSignal, &weight, false, Scale_UUBAR_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((DDBAR_input_train + std::string("/") + names.at(i)).c_str(), input_vars, &IsSignal, &weight, false, Scale_DDBAR_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SSBAR_input_train + std::string("/") + names.at(i)).c_str(), input_vars, &IsSignal, &weight, false, Scale_SSBAR_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((CHARM_input_train + std::string("/") + names.at(i)).c_str(), input_vars, &IsSignal, &weight, false, Scale_CHARM_train, "otherwise");
        }
    }
...

    {
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((SIGNAL_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, true, Scale_Kplus_test, "B2Knunu");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((SIGNAL_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, true, Scale_Kplusstar_test, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((SIGNAL_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, true, Scale_Xsu_nonresonant_test, "B2Xsnunu");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((SIGNAL_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, true, Scale_K0_test, "B02K0nunu");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((SIGNAL_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, true, Scale_K0star_test, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((SIGNAL_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, true, Scale_Xsd_nonresonant_test, "B02Xsnunu");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((CHG_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, false, Scale_CHG_test, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((MIX_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, false, Scale_MIX_test, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((UUBAR_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, false, Scale_UUBAR_test, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((DDBAR_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, false, Scale_DDBAR_test, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((SSBAR_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, false, Scale_SSBAR_test, "otherwise");
        }
    }
    {
        std::vector<string> names;
        ...
        for (unsigned int i = 0; i < names.size(); ++i) {
            // take even only
            if (i % 2 == 1) continue;
            FillVariables((CHARM_input_test + std::string("/") + names.at(i)).c_str(), input_vars2, &IsSignal2, &weight2, false, Scale_CHARM_test, "otherwise");
        }
    }
...

    {
        ...
        load_files(SIGNAL_input_train, &names, "B2Knunu");
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SIGNAL_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, true, Scale_Kplus_train, "B2Knunu");
        }
    }
    {
        std::vector<string> names;
        load_files(SIGNAL_input_train, &names, "B2Kstarnunu");
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SIGNAL_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, true, Scale_Kplusstar_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        load_files(SIGNAL_input_train, &names, "B2Xsnunu");
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SIGNAL_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, true, Scale_Xsu_nonresonant_train, "B2Xsnunu");
        }
    }
    {
        std::vector<string> names;
        load_files(SIGNAL_input_train, &names, "B02K0nunu");
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SIGNAL_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, true, Scale_K0_train, "B02K0nunu");
        }
    }
    {
        std::vector<string> names;
        load_files(SIGNAL_input_train, &names, "B02Kstar0nunu");
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SIGNAL_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, true, Scale_K0star_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        load_files(SIGNAL_input_train, &names, "B02Xsnunu");
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SIGNAL_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, true, Scale_Xsd_nonresonant_train, "B02Xsnunu");
        }
    }
    {
        std::vector<string> names;
        load_files(CHG_input_train, &names);
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((CHG_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, false, Scale_CHG_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        load_files(MIX_input_train, &names);
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((MIX_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, false, Scale_MIX_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        load_files(UUBAR_input_train, &names);
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((UUBAR_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, false, Scale_UUBAR_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        load_files(DDBAR_input_train, &names);
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((DDBAR_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, false, Scale_DDBAR_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        load_files(SSBAR_input_train, &names);
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((SSBAR_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, false, Scale_SSBAR_train, "otherwise");
        }
    }
    {
        std::vector<string> names;
        load_files(CHARM_input_train, &names);
        for (unsigned int i = 0; i < names.size(); ++i) {
            FillVariables((CHARM_input_train + std::string("/") + names.at(i)).c_str(), input_vars3, &IsSignal3, &weight3, false, Scale_CHARM_train, "otherwise");
        }
    }

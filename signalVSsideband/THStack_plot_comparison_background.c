...
void LetsFill_ri(const char* dirname, std::vector<std::string> variable_names, std::vector<std::string> branch_names, std::vector<double> variable_values[Nvar_num], std::vector<int>* numberings, std::vector<double>* weights, std::string SampleName, std::string region, std::string sample_type, double CAL, double CAL_qq) {
    /*
    SampleName for Knn
    CHG
    MIX
    ...
    */
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
            ...
            if (SampleName == "CHG" && region == "signal") {
                numberings->push_back(0);
                FEI_calibration_factor = corrector_FEI.GetFEICalFactor(Upsilon_ID, Btag_ID, MCTYPE);
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "MIX" && region == "signal") {
                numberings->push_back(1);
                FEI_calibration_factor = corrector_FEI.GetFEICalFactor(Upsilon_ID, Btag_ID, MCTYPE);
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "UUBAR" && region == "signal") {
                numberings->push_back(2);
                FEI_calibration_factor = CAL_qq;
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "DDBAR" && region == "signal") {
                numberings->push_back(3);
                FEI_calibration_factor = CAL_qq;
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "SSBAR" && region == "signal") {
                numberings->push_back(4);
                FEI_calibration_factor = CAL_qq;
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "CHARM" && region == "signal") {
                numberings->push_back(5);
                FEI_calibration_factor = CAL_qq;
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "CHG" && region == "sideband") {
                numberings->push_back(0 + IndexShift);
                FEI_calibration_factor = corrector_FEI.GetFEICalFactor(Upsilon_ID, Btag_ID, MCTYPE);
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "MIX" && region == "sideband") {
                numberings->push_back(1 + IndexShift);
                FEI_calibration_factor = corrector_FEI.GetFEICalFactor(Upsilon_ID, Btag_ID, MCTYPE);
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "UUBAR" && region == "sideband") {
                numberings->push_back(2 + IndexShift);
                FEI_calibration_factor = CAL_qq;
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "DDBAR" && region == "sideband") {
                numberings->push_back(3 + IndexShift);
                FEI_calibration_factor = CAL_qq;
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "SSBAR" && region == "sideband") {
                numberings->push_back(4 + IndexShift);
                FEI_calibration_factor = CAL_qq;
                weight_ri = ObtainWeight(SampleName.c_str(), MCTYPE, sample_type.c_str(), names.at(i));
            }
            else if (SampleName == "CHARM" && region == "sideband") {
...
void NevtCount_ri(const char* dirname, std::string SampleName, Nevt* nevt, std::string region, std::string sample_type, double CAL, double CAL_qq) {
    /*
    SampleName for Knn
    CHG
    MIX
...
void THStack_plot_comparison_background() {
    ...
    const char* MC15rd_CHG_signal_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD/CHG_analysis/validation_v004/final_output";
    const char* MC15rd_MIX_signal_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD/MIX_analysis/validation_v004/final_output";
    const char* MC15rd_UUBAR_signal_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD/UUBAR_analysis/validation_v004/final_output";
    const char* MC15rd_DDBAR_signal_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD/DDBAR_analysis/validation_v004/final_output";
    const char* MC15rd_SSBAR_signal_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD/SSBAR_analysis/validation_v004/final_output";
    const char* MC15rd_CHARM_signal_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD/CHARM_analysis/validation_v004/final_output";

    const char* MC15rd_CHG_sideband_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD_LS_MC_side/CHG_analysis/validation_v004/final_output";
    const char* MC15rd_MIX_sideband_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD_LS_MC_side/MIX_analysis/validation_v004/final_output";
    const char* MC15rd_UUBAR_sideband_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD_LS_MC_side/UUBAR_analysis/validation_v004/final_output";
    const char* MC15rd_DDBAR_sideband_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD_LS_MC_side/DDBAR_analysis/validation_v004/final_output";
    const char* MC15rd_SSBAR_sideband_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD_LS_MC_side/SSBAR_analysis/validation_v004/final_output";
    const char* MC15rd_CHARM_sideband_dirname = "/home/belle2/junewoo/storage_b1/bsub/Analysis/SatoriRD_LS_MC_side/CHARM_analysis/validation_v004/final_output";
    ...
    std::vector<double> charged_signal_values[Nvar_num];
    std::vector<double> mixed_signal_values[Nvar_num];
    std::vector<double> uubar_signal_values[Nvar_num];
    std::vector<double> ddbar_signal_values[Nvar_num];
    std::vector<double> ssbar_signal_values[Nvar_num];
    std::vector<double> ccbar_signal_values[Nvar_num];
    std::vector<double> taupair_signal_values[Nvar_num];
    std::vector<double> mumu_signal_values[Nvar_num];
    std::vector<double> gg_signal_values[Nvar_num];
    std::vector<double> ee_signal_values[Nvar_num];
    std::vector<double> eeee_signal_values[Nvar_num];
    std::vector<double> eemumu_signal_values[Nvar_num];
    std::vector<double> llXX_signal_values[Nvar_num];
    std::vector<double> hhISR_signal_values[Nvar_num];
    std::vector<double> charged_sideband_values[Nvar_num];
    std::vector<double> mixed_sideband_values[Nvar_num];
    std::vector<double> uubar_sideband_values[Nvar_num];
    std::vector<double> ddbar_sideband_values[Nvar_num];
    std::vector<double> ssbar_sideband_values[Nvar_num];
    std::vector<double> ccbar_sideband_values[Nvar_num];
    std::vector<double> signal_values[Nvar_num];
    std::vector<double> sideband_values[Nvar_num];
    std::vector<int> MC_numbering;

    std::vector<double> data_values[Nvar_num];

    std::vector<double> weights;
    std::vector<double> charged_signal_weights;
    std::vector<double> mixed_signal_weights;
    std::vector<double> uubar_signal_weights;
    std::vector<double> ddbar_signal_weights;
    std::vector<double> ssbar_signal_weights;
    std::vector<double> ccbar_signal_weights;
    std::vector<double> taupair_signal_weights;
    std::vector<double> mumu_signal_weights;
    std::vector<double> gg_signal_weights;
    std::vector<double> ee_signal_weights;
    std::vector<double> eeee_signal_weights;
    std::vector<double> eemumu_signal_weights;
    std::vector<double> llXX_signal_weights;
    std::vector<double> hhISR_signal_weights;
    std::vector<double> charged_sideband_weight;
    std::vector<double> mixed_sideband_weight;
    std::vector<double> uubar_sideband_weight;
    std::vector<double> ddbar_sideband_weight;
    std::vector<double> ssbar_sideband_weight;
    std::vector<double> ccbar_sideband_weight;
    ...
    for (int k = 0; k < (int)MC_numbering.size(); k++) {
        if (MC_numbering.at(k) == 0) {
            for (int l = 0; l < (int)variable_names.size(); l++) charged_signal_values[l].push_back(MC_values[l].at(k));
            charged_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 1) {
            for (int l = 0; l < (int)variable_names.size(); l++) mixed_signal_values[l].push_back(MC_values[l].at(k));
            mixed_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 2) {
            for (int l = 0; l < (int)variable_names.size(); l++) uubar_signal_values[l].push_back(MC_values[l].at(k));
            uubar_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 3) {
            for (int l = 0; l < (int)variable_names.size(); l++) ddbar_signal_values[l].push_back(MC_values[l].at(k));
            ddbar_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 4) {
            for (int l = 0; l < (int)variable_names.size(); l++) ssbar_signal_values[l].push_back(MC_values[l].at(k));
            ssbar_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 5) {
            for (int l = 0; l < (int)variable_names.size(); l++) ccbar_signal_values[l].push_back(MC_values[l].at(k));
            ccbar_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 6) {
            for (int l = 0; l < (int)variable_names.size(); l++) taupair_signal_values[l].push_back(MC_values[l].at(k));
            taupair_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 7) {
            for (int l = 0; l < (int)variable_names.size(); l++) mumu_signal_values[l].push_back(MC_values[l].at(k));
            mumu_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 8) {
            for (int l = 0; l < (int)variable_names.size(); l++) gg_signal_values[l].push_back(MC_values[l].at(k));
            gg_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 9) {
            for (int l = 0; l < (int)variable_names.size(); l++) ee_signal_values[l].push_back(MC_values[l].at(k));
            ee_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 10) {
            for (int l = 0; l < (int)variable_names.size(); l++) eeee_signal_values[l].push_back(MC_values[l].at(k));
            eeee_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 11) {
            for (int l = 0; l < (int)variable_names.size(); l++) eemumu_signal_values[l].push_back(MC_values[l].at(k));
            eemumu_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 12) {
            for (int l = 0; l < (int)variable_names.size(); l++) llXX_signal_values[l].push_back(MC_values[l].at(k));
            llXX_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == 13) {
            for (int l = 0; l < (int)variable_names.size(); l++) hhISR_signal_values[l].push_back(MC_values[l].at(k));
            hhISR_signal_weights.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == (0 + IndexShift)) {
            for (int l = 0; l < (int)variable_names.size(); l++) charged_sideband_values[l].push_back(MC_values[l].at(k));
            charged_sideband_weight.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == (1 + IndexShift)) {
            for (int l = 0; l < (int)variable_names.size(); l++) mixed_sideband_values[l].push_back(MC_values[l].at(k));
            mixed_sideband_weight.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == (2 + IndexShift)) {
            for (int l = 0; l < (int)variable_names.size(); l++) uubar_sideband_values[l].push_back(MC_values[l].at(k));
            uubar_sideband_weight.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == (3 + IndexShift)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ddbar_sideband_values[l].push_back(MC_values[l].at(k));
            ddbar_sideband_weight.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == (4 + IndexShift)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ssbar_sideband_values[l].push_back(MC_values[l].at(k));
            ssbar_sideband_weight.push_back(weights.at(k));
        }
        else if (MC_numbering.at(k) == (5 + IndexShift)) {
            for (int l = 0; l < (int)variable_names.size(); l++) ccbar_sideband_values[l].push_back(MC_values[l].at(k));
            ccbar_sideband_weight.push_back(weights.at(k));
        }
        else {
        ...
        if (MC_numbering.at(k) < IndexShift) { // MC15ri
            for (int l = 0; l < (int)variable_names.size(); l++) signal_values[l].push_back(MC_values[l].at(k));
            signal_weights.push_back(weights.at(k));
        }
        else { // MC15rd
            for (int l = 0; l < (int)variable_names.size(); l++) sideband_values[l].push_back(MC_values[l].at(k));
            sideband_weight.push_back(weights.at(k));
        }
    }

    THStack* Stack[Nvar_num];
    TH1D* charged_signal_hist[Nvar_num];
    TH1D* mixed_signal_hist[Nvar_num];
    TH1D* uubar_signal_hist[Nvar_num];
    TH1D* ddbar_signal_hist[Nvar_num];
    TH1D* ssbar_signal_hist[Nvar_num];
    TH1D* ccbar_signal_hist[Nvar_num];
    TH1D* taupair_signal_hist[Nvar_num];
    TH1D* mumu_signal_hist[Nvar_num];
    TH1D* gg_signal_hist[Nvar_num];
    TH1D* ee_signal_hist[Nvar_num];
    TH1D* eeee_signal_hist[Nvar_num];
    TH1D* eemumu_signal_hist[Nvar_num];
    TH1D* llXX_signal_hist[Nvar_num];
    TH1D* hhISR_signal_hist[Nvar_num];
    TH1D* charged_sideband_hist[Nvar_num];
    TH1D* mixed_sideband_hist[Nvar_num];
    TH1D* uubar_sideband_hist[Nvar_num];
    TH1D* ddbar_sideband_hist[Nvar_num];
    TH1D* ssbar_sideband_hist[Nvar_num];
    TH1D* ccbar_sideband_hist[Nvar_num];
    TH1D* signal_hist[Nvar_num];
    TH1D* sideband_hist[Nvar_num];
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // malloc TH1D
        ...
        temp_v.insert(temp_v.end(), charged_signal_values[k].begin(), charged_signal_values[k].end());
        temp_v.insert(temp_v.end(), mixed_signal_values[k].begin(), mixed_signal_values[k].end());
        temp_v.insert(temp_v.end(), uubar_signal_values[k].begin(), uubar_signal_values[k].end());
        temp_v.insert(temp_v.end(), ddbar_signal_values[k].begin(), ddbar_signal_values[k].end());
        temp_v.insert(temp_v.end(), ssbar_signal_values[k].begin(), ssbar_signal_values[k].end());
        temp_v.insert(temp_v.end(), ccbar_signal_values[k].begin(), ccbar_signal_values[k].end());
        temp_v.insert(temp_v.end(), taupair_signal_values[k].begin(), taupair_signal_values[k].end());
        temp_v.insert(temp_v.end(), mumu_signal_values[k].begin(), mumu_signal_values[k].end());
        temp_v.insert(temp_v.end(), gg_signal_values[k].begin(), gg_signal_values[k].end());
        temp_v.insert(temp_v.end(), ee_signal_values[k].begin(), ee_signal_values[k].end());
        temp_v.insert(temp_v.end(), eeee_signal_values[k].begin(), eeee_signal_values[k].end());
        temp_v.insert(temp_v.end(), eemumu_signal_values[k].begin(), eemumu_signal_values[k].end());
        temp_v.insert(temp_v.end(), llXX_signal_values[k].begin(), llXX_signal_values[k].end());
        temp_v.insert(temp_v.end(), hhISR_signal_values[k].begin(), hhISR_signal_values[k].end());
        temp_v.insert(temp_v.end(), charged_sideband_values[k].begin(), charged_sideband_values[k].end());
        temp_v.insert(temp_v.end(), mixed_sideband_values[k].begin(), mixed_sideband_values[k].end());
        temp_v.insert(temp_v.end(), uubar_sideband_values[k].begin(), uubar_sideband_values[k].end());
        temp_v.insert(temp_v.end(), ddbar_sideband_values[k].begin(), ddbar_sideband_values[k].end());
        temp_v.insert(temp_v.end(), ssbar_sideband_values[k].begin(), ssbar_sideband_values[k].end());
        temp_v.insert(temp_v.end(), ccbar_sideband_values[k].begin(), ccbar_sideband_values[k].end());
        ...
        charged_signal_hist[k] = new TH1D("charged signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        mixed_signal_hist[k] = new TH1D("mixed signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        uubar_signal_hist[k] = new TH1D("u#bar{u} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ddbar_signal_hist[k] = new TH1D("d#bar{d} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ssbar_signal_hist[k] = new TH1D("s#bar{s} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ccbar_signal_hist[k] = new TH1D("c#bar{c} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        taupair_signal_hist[k] = new TH1D("#tau#bar{#tau} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        mumu_signal_hist[k] = new TH1D("#mu#bar{#mu} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        gg_signal_hist[k] = new TH1D("gg signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ee_signal_hist[k] = new TH1D("e#bar{e} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        eeee_signal_hist[k] = new TH1D("ee#bar{e}#bar{e} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        eemumu_signal_hist[k] = new TH1D("e#bar{e}#mu#bar{#mu} signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        llXX_signal_hist[k] = new TH1D("\ell#bar{\ell}XX signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        hhISR_signal_hist[k] = new TH1D("hhISR signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        charged_sideband_hist[k] = new TH1D("charged sideband", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        mixed_sideband_hist[k] = new TH1D("mixed sideband", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        uubar_sideband_hist[k] = new TH1D("u#bar{u} sideband", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ddbar_sideband_hist[k] = new TH1D("d#bar{d} sideband", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ssbar_sideband_hist[k] = new TH1D("s#bar{s} sideband", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ccbar_sideband_hist[k] = new TH1D("c#bar{c} sideband", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        signal_hist[k] = new TH1D("signal region", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        sideband_hist[k] = new TH1D("sideband", (";" + variable_names.at(k) + ";arbitrary unit").c_str(), bins, min, max);
        ...
        Ratio_hist[k] = new TH1D((variable_names.at(k) + "_ratio").c_str(), ";;sideband/signal_region", bins, min, max);
    }

    int index = std::find(variable_names.begin(), variable_names.end(), std::string("log_{10}SignalProbability")) - variable_names.begin();
    for (int i = 0; i < (int)charged_signal_values[index].size(); i++) charged_signal_values[index].at(i) = log10l(charged_signal_values[index].at(i));
    for (int i = 0; i < (int)mixed_signal_values[index].size(); i++) mixed_signal_values[index].at(i) = log10l(mixed_signal_values[index].at(i));
    for (int i = 0; i < (int)uubar_signal_values[index].size(); i++) uubar_signal_values[index].at(i) = log10l(uubar_signal_values[index].at(i));
    for (int i = 0; i < (int)ddbar_signal_values[index].size(); i++) ddbar_signal_values[index].at(i) = log10l(ddbar_signal_values[index].at(i));
    for (int i = 0; i < (int)ssbar_signal_values[index].size(); i++) ssbar_signal_values[index].at(i) = log10l(ssbar_signal_values[index].at(i));
    for (int i = 0; i < (int)ccbar_signal_values[index].size(); i++) ccbar_signal_values[index].at(i) = log10l(ccbar_signal_values[index].at(i));
    for (int i = 0; i < (int)taupair_signal_values[index].size(); i++) taupair_signal_values[index].at(i) = log10l(taupair_signal_values[index].at(i));
    for (int i = 0; i < (int)mumu_signal_values[index].size(); i++) mumu_signal_values[index].at(i) = log10l(mumu_signal_values[index].at(i));
    for (int i = 0; i < (int)gg_signal_values[index].size(); i++) gg_signal_values[index].at(i) = log10l(gg_signal_values[index].at(i));
    for (int i = 0; i < (int)ee_signal_values[index].size(); i++) ee_signal_values[index].at(i) = log10l(ee_signal_values[index].at(i));
    for (int i = 0; i < (int)eeee_signal_values[index].size(); i++) eeee_signal_values[index].at(i) = log10l(eeee_signal_values[index].at(i));
    for (int i = 0; i < (int)eemumu_signal_values[index].size(); i++) eemumu_signal_values[index].at(i) = log10l(eemumu_signal_values[index].at(i));
    for (int i = 0; i < (int)llXX_signal_values[index].size(); i++) llXX_signal_values[index].at(i) = log10l(llXX_signal_values[index].at(i));
    for (int i = 0; i < (int)hhISR_signal_values[index].size(); i++) hhISR_signal_values[index].at(i) = log10l(hhISR_signal_values[index].at(i));
    for (int i = 0; i < (int)charged_sideband_values[index].size(); i++) charged_sideband_values[index].at(i) = log10l(charged_sideband_values[index].at(i));
    for (int i = 0; i < (int)mixed_sideband_values[index].size(); i++) mixed_sideband_values[index].at(i) = log10l(mixed_sideband_values[index].at(i));
    for (int i = 0; i < (int)uubar_sideband_values[index].size(); i++) uubar_sideband_values[index].at(i) = log10l(uubar_sideband_values[index].at(i));
    for (int i = 0; i < (int)ddbar_sideband_values[index].size(); i++) ddbar_sideband_values[index].at(i) = log10l(ddbar_sideband_values[index].at(i));
    for (int i = 0; i < (int)ssbar_sideband_values[index].size(); i++) ssbar_sideband_values[index].at(i) = log10l(ssbar_sideband_values[index].at(i));
    for (int i = 0; i < (int)ccbar_sideband_values[index].size(); i++) ccbar_sideband_values[index].at(i) = log10l(ccbar_sideband_values[index].at(i));
    for (int i = 0; i < (int)signal_values[index].size(); i++) signal_values[index].at(i) = log10l(signal_values[index].at(i));
    for (int i = 0; i < (int)sideband_values[index].size(); i++) sideband_values[index].at(i) = log10l(sideband_values[index].at(i));
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // fill
        for (int i = 0; i < (int)charged_signal_values[k].size(); i++) charged_signal_hist[k]->Fill(charged_signal_values[k].at(i), charged_signal_weights.at(i));
        for (int i = 0; i < (int)mixed_signal_values[k].size(); i++) mixed_signal_hist[k]->Fill(mixed_signal_values[k].at(i), mixed_signal_weights.at(i));
        for (int i = 0; i < (int)uubar_signal_values[k].size(); i++) uubar_signal_hist[k]->Fill(uubar_signal_values[k].at(i), uubar_signal_weights.at(i));
        for (int i = 0; i < (int)ddbar_signal_values[k].size(); i++) ddbar_signal_hist[k]->Fill(ddbar_signal_values[k].at(i), ddbar_signal_weights.at(i));
        for (int i = 0; i < (int)ssbar_signal_values[k].size(); i++) ssbar_signal_hist[k]->Fill(ssbar_signal_values[k].at(i), ssbar_signal_weights.at(i));
        for (int i = 0; i < (int)ccbar_signal_values[k].size(); i++) ccbar_signal_hist[k]->Fill(ccbar_signal_values[k].at(i), ccbar_signal_weights.at(i));
        for (int i = 0; i < (int)taupair_signal_values[k].size(); i++) taupair_signal_hist[k]->Fill(taupair_signal_values[k].at(i), taupair_signal_weights.at(i));
        for (int i = 0; i < (int)mumu_signal_values[k].size(); i++) mumu_signal_hist[k]->Fill(mumu_signal_values[k].at(i), mumu_signal_weights.at(i));
        for (int i = 0; i < (int)gg_signal_values[k].size(); i++) gg_signal_hist[k]->Fill(gg_signal_values[k].at(i), gg_signal_weights.at(i));
        for (int i = 0; i < (int)ee_signal_values[k].size(); i++) ee_signal_hist[k]->Fill(ee_signal_values[k].at(i), ee_signal_weights.at(i));
        for (int i = 0; i < (int)eeee_signal_values[k].size(); i++) eeee_signal_hist[k]->Fill(eeee_signal_values[k].at(i), eeee_signal_weights.at(i));
        for (int i = 0; i < (int)eemumu_signal_values[k].size(); i++) eemumu_signal_hist[k]->Fill(eemumu_signal_values[k].at(i), eemumu_signal_weights.at(i));
        for (int i = 0; i < (int)llXX_signal_values[k].size(); i++) llXX_signal_hist[k]->Fill(llXX_signal_values[k].at(i), llXX_signal_weights.at(i));
        for (int i = 0; i < (int)hhISR_signal_values[k].size(); i++) hhISR_signal_hist[k]->Fill(hhISR_signal_values[k].at(i), hhISR_signal_weights.at(i));
        for (int i = 0; i < (int)charged_sideband_values[k].size(); i++) charged_sideband_hist[k]->Fill(charged_sideband_values[k].at(i), charged_sideband_weight.at(i));
        for (int i = 0; i < (int)mixed_sideband_values[k].size(); i++) mixed_sideband_hist[k]->Fill(mixed_sideband_values[k].at(i), mixed_sideband_weight.at(i));
        for (int i = 0; i < (int)uubar_sideband_values[k].size(); i++) uubar_sideband_hist[k]->Fill(uubar_sideband_values[k].at(i), uubar_sideband_weight.at(i));
        for (int i = 0; i < (int)ddbar_sideband_values[k].size(); i++) ddbar_sideband_hist[k]->Fill(ddbar_sideband_values[k].at(i), ddbar_sideband_weight.at(i));
        for (int i = 0; i < (int)ssbar_sideband_values[k].size(); i++) ssbar_sideband_hist[k]->Fill(ssbar_sideband_values[k].at(i), ssbar_sideband_weight.at(i));
        for (int i = 0; i < (int)ccbar_sideband_values[k].size(); i++) ccbar_sideband_hist[k]->Fill(ccbar_sideband_values[k].at(i), ccbar_sideband_weight.at(i));
        for (int i = 0; i < (int)signal_values[k].size(); i++) signal_hist[k]->Fill(signal_values[k].at(i), signal_weights.at(i));
        for (int i = 0; i < (int)sideband_values[k].size(); i++) sideband_hist[k]->Fill(sideband_values[k].at(i), sideband_weight.at(i));
    ...
    for (int k = 0; k < (int)variable_names.size(); k++) { // draw
        ...
        Float_t ymax_1 = sideband_hist[k]->GetMaximum();
        Float_t ymax_2 = signal_hist[k]->GetMaximum();
        double real_max = 0;
        if (ymax_1 > ymax_2) real_max = ymax_1;
        else real_max = ymax_2;

        sideband_hist[k]->SetMaximum(real_max * 1.1);

        signal_hist[k]->SetStats(0);
        sideband_hist[k]->SetStats(0);

        sideband_hist[k]->SetFillColor(4); sideband_hist[k]->SetLineColor(4); sideband_hist[k]->SetLineWidth(1); sideband_hist[k]->SetFillStyle(3002); sideband_hist[k]->Draw("e1 Hist");
        signal_hist[k]->SetFillColor(2); signal_hist[k]->SetLineColor(2); signal_hist[k]->SetLineWidth(1); signal_hist[k]->SetFillStyle(3003); signal_hist[k]->Draw("e1 Hist SAME");
        ...
        c_temp->SaveAs((variable_names.at(k) + "_signal_vs_sideband.png").c_str());
    ...
    double signal_sum = 0;
    double sideband_sum = 0;
    for (int i = 0; i < (int)signal_values[0].size(); i++) signal_sum = signal_sum + signal_weights.at(i);
    for (int i = 0; i < (int)sideband_values[0].size(); i++) sideband_sum = sideband_sum + sideband_weight.at(i);
    printf("data num: %ld\n", data_values[0].size());
    printf("signal region num with calibration: %lf\n", signal_sum);
    printf("sideband num with calibration: %lf\n", sideband_sum);
}

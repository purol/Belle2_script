...
void Initialize_options(OPTIONS* options_) {
...
void FixParameters(RooWorkspace* w) {
    ...
    // track
    w->var("alpha_track_eff_uncer")->setConstant(!options->track);

    // PID
    for (int i = 0; i < NEntryPID; i++) w->var(("alpha_PID" + std::to_string(i) + "_uncer").c_str())->setConstant(!options->PID);

    // KID
    for (int i = 0; i < NEntryKID; i++) w->var(("alpha_KID" + std::to_string(i) + "_uncer").c_str())->setConstant(!options->KID);

    // KS0
    w->var("alpha_KS0_reco_uncer")->setConstant(!options->KS0);

    // pi0
    for (int i = 0; i < NEntrypi0; i++) w->var(("alpha_pi0" + std::to_string(i) + "_uncer").c_str())->setConstant(!options->pi0);

    // FBDT efficiency
    w->var("alpha_FBDT_efficiency_uncer")->setConstant(!options->FBDT);

    // FEI
    for (int i = 0; i < NEntryFEI; i++) w->var(("alpha_FEI" + std::to_string(i) + "_uncer").c_str())->setConstant(!options->FEI);

    // qqbar normalization
    w->var("alpha_qq_CAL_UUBAR_uncer")->setConstant(!options->qqbar);
    w->var("alpha_qq_CAL_DDBAR_uncer")->setConstant(!options->qqbar);
    w->var("alpha_qq_CAL_SSBAR_uncer")->setConstant(!options->qqbar);
    w->var("alpha_qq_CAL_CHARM_uncer")->setConstant(!options->qqbar);

    // photon multiplicity correction
    for (int i = 0; i < NEntryMultiplicity; i++) w->var("")->setConstant(!options->multiplicity);
}

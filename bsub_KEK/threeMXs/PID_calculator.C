...
double PID_fakeE_correction_fluctuated[4][N_fakeE_syst] = { 0.0 }; //  K-, K+, pi-, pi+
double PID_fakeMU_correction_fluctuated[4][N_fakeMU_syst] = { 0.0 }; //  K-, K+, pi-, pi+
...
void GetNominalNevt(const char* dirname, const char* included_string, TH1D* histMX1, TH1D* histMX2, TH1D* histMX3, const char* type, const char* sample, double Nevt_nominal[RarityBins * 7 * 3], double weight_var = 1.0, std::string CorrectionType = "otherwise") { // get nominal PDF with appropriate correction
    /*
    ...
    */
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
            ...
            if ((Bsig_M > 0) && (Bsig_M < 0.6)) histMX1->Fill(MVA_var, total_weight);
            else if ((Bsig_M > 0.6) && (Bsig_M < 1.0)) histMX2->Fill(MVA_var, total_weight);
            else histMX3->Fill(MVA_var, total_weight);
        }
        input_file->Close();
...
void GetFlucNevt(const char* dirname, const char* included_string, TH1D* histMX1, TH1D* histMX2, TH1D* histMX3, const char* type, const char* sample, double Nevt_fluc[NToys][RarityBins * 7 * 3], int ToyNum, bool IsItKID, double weight_var = 1.0, std::string CorrectionType = "otherwise") { // get nominal PDF with appropriate correction
    /*
    CorrectionType for new form factors
    B2Knunu
    B02K0nunu
    ...
    */
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        for (unsigned int j = 0; j < tree_upsilon->GetEntries(); j++) { // Fill
            ...
            for (int i_pi0 = 0; i_pi0 < N_pi0_syst; i_pi0++) Correction_pi0 = Correction_pi0 * std::pow(corrector_pi0.GetCorrectionFactor(i_pi0, MCTYPE), temp_N_bin_pi0[i_pi0]);
            for (int i_fake = 0; i_fake < N_fakeE_syst; i_fake++) {
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeE(0, i_fake, MCTYPE) * PID_fakeE_correction_fluctuated[0][i_fake], temp_N_bin_fakeE[0][i_fake]); // K- from e
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeE(1, i_fake, MCTYPE) * PID_fakeE_correction_fluctuated[1][i_fake], temp_N_bin_fakeE[1][i_fake]); // K+ from e
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeE(2, i_fake, MCTYPE) * PID_fakeE_correction_fluctuated[2][i_fake], temp_N_bin_fakeE[2][i_fake]); // pi- from e
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeE(3, i_fake, MCTYPE) * PID_fakeE_correction_fluctuated[3][i_fake], temp_N_bin_fakeE[3][i_fake]); // pi+ from e
            }
            for (int i_fake = 0; i_fake < N_fakeMU_syst; i_fake++) {
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeMU(0, i_fake, MCTYPE) * PID_fakeMU_correction_fluctuated[0][i_fake], temp_N_bin_fakeMU[0][i_fake]); // K- from mu
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeMU(1, i_fake, MCTYPE) * PID_fakeMU_correction_fluctuated[1][i_fake], temp_N_bin_fakeMU[1][i_fake]); // K+ from mu
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeMU(2, i_fake, MCTYPE) * PID_fakeMU_correction_fluctuated[2][i_fake], temp_N_bin_fakeMU[2][i_fake]); // pi- from mu
                Correction_fake = Correction_fake * std::pow(corrector_FakePID.GetCorrectionFactorfakeMU(3, i_fake, MCTYPE) * PID_fakeMU_correction_fluctuated[3][i_fake], temp_N_bin_fakeMU[3][i_fake]); // pi+ from mu
            }
...
void GetFlucNevt(const char* dirname, const char* included_string, TH1D* histMX1, TH1D* histMX2, TH1D* histMX3, const char* type, const char* sample, double Nevt_fluc[NToys][RarityBins * 7 * 3], int ToyNum, double weight_var = 1.0, std::string CorrectionType = "otherwise") { // get nominal PDF with appropriate correction
    /*
    CorrectionType for new form factors
    B2Knunu
    B02K0nunu
...
void FluctuatePIDCorrection(bool IsItKID) {
    ...
    for (int i_fake = 0; i_fake < N_fakeE_syst; i_fake++) {
        ...
        std::lognormal_distribution<double> Kp_fakeE_distribution(0.0, corrector_FakePID.GetUncertaintyfakeE(1, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeE(1, i_fake, MCTYPE));
        std::lognormal_distribution<double> Kn_fakeE_distribution(0.0, corrector_FakePID.GetUncertaintyfakeE(0, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeE(0, i_fake, MCTYPE));
        std::lognormal_distribution<double> pip_fakeE_distribution(0.0, corrector_FakePID.GetUncertaintyfakeE(3, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeE(3, i_fake, MCTYPE));
        std::lognormal_distribution<double> pin_fakeE_distribution(0.0, corrector_FakePID.GetUncertaintyfakeE(2, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeE(2, i_fake, MCTYPE));
    ...
    for (int i_fake = 0; i_fake < N_fakeMU_syst; i_fake++) {
        ...
        std::lognormal_distribution<double> Kp_fakeMU_distribution(0.0, corrector_FakePID.GetUncertaintyfakeMU(1, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeMU(1, i_fake, MCTYPE));
        std::lognormal_distribution<double> Kn_fakeMU_distribution(0.0, corrector_FakePID.GetUncertaintyfakeMU(0, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeMU(0, i_fake, MCTYPE));
        std::lognormal_distribution<double> pip_fakeMU_distribution(0.0, corrector_FakePID.GetUncertaintyfakeMU(3, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeMU(3, i_fake, MCTYPE));
        std::lognormal_distribution<double> pin_fakeMU_distribution(0.0, corrector_FakePID.GetUncertaintyfakeMU(2, i_fake, MCTYPE) / corrector_FakePID.GetCorrectionFactorfakeMU(2, i_fake, MCTYPE));
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    double Nevt_nominal[RarityBins * 7 * 3] = { 0.0 }; // CHG MIX UUBAR DDBAR SSBAR CHARM SIGNAL
    double Nevt_fluc_KID[NToys][RarityBins * 7 * 3] = { 0.0 }; // CHG MIX UUBAR DDBAR SSBAR CHARM SIGNAL
    double Nevt_fluc_PID[NToys][RarityBins * 7 * 3] = { 0.0 }; // CHG MIX UUBAR DDBAR SSBAR CHARM SIGNAL
    double Relative_Uncertainty_KID[NToys][RarityBins * 7 * 3] = { 0.0 };
    double Relative_Uncertainty_PID[NToys][RarityBins * 7 * 3] = { 0.0 };
    double Covariance_KID[RarityBins * 7 * 3][RarityBins * 7 * 3] = { 0.0 };
    double Covariance_PID[RarityBins * 7 * 3][RarityBins * 7 * 3] = { 0.0 };



    /* ====================================== */
    // define TH1D for temporary usage
    TH1D* temp_hist_MX1 = new TH1D("temp_hist_MX1", "temp_hist_MX1", RarityBins, 0.96, 1.0);
    TH1D* temp_hist_MX2 = new TH1D("temp_hist_MX2", "temp_hist_MX2", RarityBins, 0.96, 1.0);
    TH1D* temp_hist_MX3 = new TH1D("temp_hist_MX3", "temp_hist_MX3", RarityBins, 0.96, 1.0);
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    /* ====================================== */
    ...
    /* ====================================== */
    // get nominal Nevt
    GetNominalNevt(MC_dirname_SIGNAL, "B2Knunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_nominal, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Knunu"), "B2Knunu");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_SIGNAL, "B2Kstarnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_nominal, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Kstarnunu"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_SIGNAL, "B2Xsnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_nominal, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Xsnunu"), "B2Xsnunu");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_SIGNAL, "B02K0nunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_nominal, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02K0nunu"), "B02K0nunu");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_SIGNAL, "B02Kstar0nunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_nominal, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Kstar0nunu"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_SIGNAL, "B02Xsnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_nominal, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Xsnunu"), "B02Xsnunu");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();

    GetNominalNevt(MC_dirname_CHG, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "CHG", Nevt_nominal, ObtainWeight("CHG", MCTYPE, "validation", "CHG"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_MIX, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "MIX", Nevt_nominal, ObtainWeight("MIX", MCTYPE, "validation", "MIX"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_UUBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "UUBAR", Nevt_nominal, ObtainWeight("UUBAR", MCTYPE, "validation", "UUBAR"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_DDBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "DDBAR", Nevt_nominal, ObtainWeight("DDBAR", MCTYPE, "validation", "DDBAR"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_SSBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "SSBAR", Nevt_nominal, ObtainWeight("SSBAR", MCTYPE, "validation", "SSBAR"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    GetNominalNevt(MC_dirname_CHARM, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "CHARM", Nevt_nominal, ObtainWeight("CHARM", MCTYPE, "validation", "CHARM"), "otherwise");
    temp_hist_MX1->Reset();
    temp_hist_MX2->Reset();
    temp_hist_MX3->Reset();
    /* ====================================== */
    ...
    for (int i = 0; i < NToys; i++) {
        ...
        GetFlucNevt(MC_dirname_SIGNAL, "B2Knunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_fluc_KID, i, true, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Knunu"), "B2Knunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B2Kstarnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_fluc_KID, i, true, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Kstarnunu"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B2Xsnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_fluc_KID, i, true, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Xsnunu"), "B2Xsnunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B02K0nunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_fluc_KID, i, true, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02K0nunu"), "B02K0nunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B02Kstar0nunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_fluc_KID, i, true, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Kstar0nunu"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B02Xsnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_fluc_KID, i, true, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Xsnunu"), "B02Xsnunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();

        GetFlucNevt(MC_dirname_CHG, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "CHG", Nevt_fluc_KID, i, true, ObtainWeight("CHG", MCTYPE, "validation", "CHG"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_MIX, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "MIX", Nevt_fluc_KID, i, true, ObtainWeight("MIX", MCTYPE, "validation", "MIX"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_UUBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "UUBAR", Nevt_fluc_KID, i, true, ObtainWeight("UUBAR", MCTYPE, "validation", "UUBAR"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_DDBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "DDBAR", Nevt_fluc_KID, i, true, ObtainWeight("DDBAR", MCTYPE, "validation", "DDBAR"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SSBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "SSBAR", Nevt_fluc_KID, i, true, ObtainWeight("SSBAR", MCTYPE, "validation", "SSBAR"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_CHARM, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "CHARM", Nevt_fluc_KID, i, true, ObtainWeight("CHARM", MCTYPE, "validation", "CHARM"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
    }
    ...
    for (int i = 0; i < NToys; i++) {
        ...
        GetFlucNevt(MC_dirname_SIGNAL, "B2Knunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_fluc_PID, i, false, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Knunu"), "B2Knunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B2Kstarnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_fluc_PID, i, false, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Kstarnunu"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B2Xsnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "SIGNAL", Nevt_fluc_PID, i, false, ObtainWeight("SIGNAL", MCTYPE, "validation", "B2Xsnunu"), "B2Xsnunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B02K0nunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_fluc_PID, i, false, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02K0nunu"), "B02K0nunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B02Kstar0nunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_fluc_PID, i, false, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Kstar0nunu"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SIGNAL, "B02Xsnunu", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "SIGNAL", Nevt_fluc_PID, i, false, ObtainWeight("SIGNAL", MCTYPE, "validation", "B02Xsnunu"), "B02Xsnunu");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();

        GetFlucNevt(MC_dirname_CHG, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bplus", "CHG", Nevt_fluc_PID, i, false, ObtainWeight("CHG", MCTYPE, "validation", "CHG"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_MIX, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Bzero", "MIX", Nevt_fluc_PID, i, false, ObtainWeight("MIX", MCTYPE, "validation", "MIX"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_UUBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "UUBAR", Nevt_fluc_PID, i, false, ObtainWeight("UUBAR", MCTYPE, "validation", "UUBAR"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_DDBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "DDBAR", Nevt_fluc_PID, i, false, ObtainWeight("DDBAR", MCTYPE, "validation", "DDBAR"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_SSBAR, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "SSBAR", Nevt_fluc_PID, i, false, ObtainWeight("SSBAR", MCTYPE, "validation", "SSBAR"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
        GetFlucNevt(MC_dirname_CHARM, "root", temp_hist_MX1, temp_hist_MX2, temp_hist_MX3, "Continuum", "CHARM", Nevt_fluc_PID, i, false, ObtainWeight("CHARM", MCTYPE, "validation", "CHARM"), "otherwise");
        temp_hist_MX1->Reset();
        temp_hist_MX2->Reset();
        temp_hist_MX3->Reset();
    }
    ...
    for (int i = 0; i < NToys; i++) {
        for (int j = 0; j < RarityBins * 7 * 3; j++) {

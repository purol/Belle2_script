...
public:
    ...
    double FluctuateCorrection(double Decay[N_decay], double MXs, SystType systtype, int TargetCategory, bool IsTargetCategoryUp, Sample sample, std::string type);
};
...
double Corrector_Fragmentation::FluctuateCorrection(double Decay[N_decay], double MXs, SystType systtype, int TargetCategory, bool IsTargetCategoryUp, Sample sample, std::string type) {
    /*
    ...
    while (!temp_queue.empty()) {
        ...
        if (smart_mode == false) temp_hist[decaymodeid]->Fill(Mxs(temp_data));
            ...
            if (filename.find("B2Knunu") != string::npos)  temp_hist[decaymodeid]->Fill(Mxs(temp_data), Scale_Kplus);
                ...
                double Correction_Fragmentation = corrector_Fragmentation.GetCorrectionFactor(temp_data.Decay, Mxs(temp_data), Corrector_Fragmentation::Nominal, Corrector_Fragmentation::gamma, "MC15ri");
